...
TEEC_Result openChannel(const hidl_vec<uint8_t> &aid, uint8_t p2, int *cNumber,
  ...
  if (!isBasic) {
    tx.c_base = MC_OPEN;
    tx.len  = sizeof(MC_OPEN) / sizeof(uint8_t);
  ...
  if ((*rLen == 3) || isBasic) {
    if (((rApdu[1] == 0x90) && (rApdu[2] == 0x00)) || isBasic) {
      tApduSize = sizeof(SELECT) / sizeof(uint8_t) + aid.size() + 1;
      ...
      memscpy(tApdu, tApduSize, SELECT, sizeof(SELECT) / sizeof(uint8_t)); //Select command
      ...
      *rLen = gpqese_transceive(&tx, &rx, &result);
      ...
      if ((*rLen > 1) && (result == TEEC_SUCCESS)) {
        memscpy(respApdu, *rLen, rApdu, sizeof(rApdu) / sizeof(uint8_t));
...
TEEC_Result closeGPChannel(uint8_t channelNumber) {
  ...
  if (channelNumber == 0) {
    tApduSize = sizeof(SELECT) / sizeof(uint8_t);
    tApdu = new (std::nothrow) uint8_t[tApduSize];
    if (NULL == tApdu) {
      ALOGE("Unable to create tApdu");
    ...
    tApduSize = sizeof(MC_CLOSE) / sizeof(uint8_t);
    tApdu = new (std::nothrow) uint8_t[tApduSize];
    if (NULL == tApdu) {
      ALOGE("Unable to create tApdu");
      return TEEC_ERROR_OUT_OF_MEMORY;
