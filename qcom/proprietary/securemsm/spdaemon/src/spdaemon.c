...
#define SP_APP_LOAD_FLAG_SILENT_LOAD    0x00080000
...
static bool             g_sp_reset_detected = false;
...
static int handle_sp_out_of_reset(void)
{
    ...
    if (service_client) {
        ...
        if (!retval) {
            LOGI("Sensors Calibration: %s, ARI mode: %d, ARI fuse: %d",
...
int main(void)
{
    ...
    ret = spcom_restart_sp_subsystem();
    ...
    while(1) {
        ...
        g_sp_reset_detected = false;
        suspend_me();
        if (g_sp_reset_detected) {
            ret = handle_sp_out_of_reset();
            if (ret < 0) {
