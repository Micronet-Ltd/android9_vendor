...
typedef struct ParserFrameInfoType
{
   ...
   uint32 nFilledLen;     //The length of filled data in the buffer
...
typedef struct FileOffset
{
    ...
    uint32 nOffset;     //Buffer data read start offset
    uint32 nLength;     //The length of the data in the buffer
    uint32 isLastFrame;
}FileOffset;
...
struct VideoStreamParser
{
   ...
   uint32   m_nFileSize;  //TODO: based on what ftell returns
   ...
   uint32   m_nCurFilePos;          // Current position of InputFile pointer.
