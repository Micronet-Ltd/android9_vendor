...
void HealthInfoLog::UpdateStatInfoBuffer(char* buf, int len)
{
  std::unique_lock<std::mutex> guard(health_info_mutex_);

  AddBuffNode(len, (uint8_t*)buf);
}
...
void HealthInfoLog::ReportHealthInfo()
{
  ...
  snprintf(stat_buf, 512, "ts <%02d:%02d:%02d:%06d> rx_vote:%0x, tx_vote:%0x, wake_lock holding:%d, "
            ...
            hour, minute, second, (unsigned int)tv.tv_usec,  hIbs->state_machine_.rx_vote,
            hIbs->state_machine_.tx_vote, lock_held, instance->max_interval_rx_sleep_wake,
            instance->max_interval_tx_packet, instance->count_wake_sent, instance->count_wake_received,
            instance->count_sleep_sent, instance->count_sleep_received);
  ALOGD("%s:statistics:\n%s",__FUNCTION__, stat_buf);
  instance->UpdateStatInfoBuffer(stat_buf, strlen(stat_buf)+1);

  //Reset statistics variables
  instance->max_interval_rx_sleep_wake = instance->max_interval_tx_packet = 0;
  instance->count_wake_sent = instance->count_wake_received = 0;
  instance->count_sleep_sent = instance->count_sleep_received = 0;
}

void HealthInfoLog::HealthStatisticsTimerStart()
{
    int status;
