...
class Logger
{
 private:
  ...
  void SaveSocMemDump(uint8_t *, uint16_t, ReasonCode);
 ...
 public:
  ...
  void DumpRingBuffers(ReasonCode reason) {
    ring_buff_.DumpData(reason);
  };
#endif
  void SaveCrashReason(ReasonCode reason);
#ifdef DUMP_IPC_LOG
...
#ifdef DUMP_RINGBUF_LOG
    ring_buff_.UpdateRxEventTag(rx_event);
#endif
  };
...
#ifdef DUMP_RINGBUF_LOG
    ring_buff_.SaveLastStackHciCommand(cmd);
#endif
  };
