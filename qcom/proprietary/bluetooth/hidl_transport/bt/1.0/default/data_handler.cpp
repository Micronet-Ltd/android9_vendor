...
DataHandler::DataHandler() {
  ...
  logger_ = Logger::Get();
  soc_always_on_enabled_ = IsSocAlwaysOnEnabled();
}
                       ...
                       DataReadCallback data_read_cb)
{
  ...
  init_thread_ = std::thread([this]() {
    ...
    if (status) {
      ...
      soc_need_reload_patch = !soc_always_on_enabled_;
      ALOGI("Firmware download succeded, soc need reload patch next time=%d\n", soc_need_reload_patch);
    } else {
...
bool DataHandler::Close(ProtocolType type)
{
  if (!controller_) {
    ...
    Wakelock :: CleanUp();
#endif
    diag_interface_.SignalEndofCleanup();
    return false;
  }
  ...
  if (!diag_interface_.SignalHALCleanup()) {
