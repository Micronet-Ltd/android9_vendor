...
int diag_qdss_init()
{
	...
	if (qdss_mask) {
		...
		while (remote_local) {
			if (remote_local & 1) {
				z = z << 1;
				qdss_periph_info.peripheral_type |= z;
				remote_local = remote_local >> 1;
			}
		}
	}
...
void* qdss_config_thread_mdm(void* param)
{
	...
	while (1) {
		if (qdss_periph_info.peripheral_type & DIAG_MDM_MASK) {
			if (mdm_peripheral_mask & DIAG_CON_MPSS) {
				diag_send_cmds_to_peripheral_init(MDM, DIAG_MODEM_PROC);
				mdm_peripheral_mask = mdm_peripheral_mask ^ DIAG_CON_MPSS;
				if (qdss_kill_thread)
					return 0;
			}
			if (mdm_peripheral_mask & DIAG_CON_LPASS) {
				diag_send_cmds_to_peripheral_init(MDM, DIAG_LPASS_PROC);
				mdm_peripheral_mask = mdm_peripheral_mask ^ DIAG_CON_LPASS;
				if (qdss_kill_thread)
					return 0;
			}
			if (mdm_peripheral_mask & DIAG_CON_WCNSS) {
				diag_send_cmds_to_peripheral_init(MDM, DIAG_WCNSS_PROC);
				mdm_peripheral_mask = mdm_peripheral_mask ^ DIAG_CON_WCNSS;
				if (qdss_kill_thread)
					return 0;
			}
			if (mdm_peripheral_mask & DIAG_CON_SENSORS) {
				diag_send_cmds_to_peripheral_init(MDM, DIAG_SENSORS_PROC);
				mdm_peripheral_mask = mdm_peripheral_mask ^ DIAG_CON_SENSORS;
				if (qdss_kill_thread)
					return 0;
			}
			if (mdm_peripheral_mask & DIAG_CON_WDSP) {
				diag_send_cmds_to_peripheral_init(MDM, DIAG_WDSP_PROC);
				mdm_peripheral_mask = mdm_peripheral_mask ^ DIAG_CON_WDSP;
				if (qdss_kill_thread)
					return 0;
			}
			if (mdm_peripheral_mask & DIAG_CON_CDSP) {
				diag_send_cmds_to_peripheral_init(MDM, DIAG_CDSP_PROC);
				mdm_peripheral_mask = mdm_peripheral_mask ^ DIAG_CON_CDSP;
				if (qdss_kill_thread)
					return 0;
			}
			if (mdm_peripheral_mask & DIAG_CON_NPU) {
				diag_send_cmds_to_peripheral_init(MDM, DIAG_NPU_PROC);
				mdm_peripheral_mask = mdm_peripheral_mask ^ DIAG_CON_NPU;
				if (qdss_kill_thread)
					return 0;
			}
			if (mdm_peripheral_mask & DIAG_CON_APSS) {
				diag_send_cmds_to_peripheral_init(MDM, DIAG_APPS_PROC);
				mdm_peripheral_mask = mdm_peripheral_mask ^ DIAG_CON_APSS;
				if (qdss_kill_thread)
					return 0;
			}
			qdss_periph_info.peripheral_type = qdss_periph_info.peripheral_type ^ DIAG_MDM_MASK;
			/************************************************************************
			 * Open Device node after setting QDSS environment by sending commands	*
			 * Signal the condition to qdss_init for successful/failure case		*
			 ************************************************************************/
...
void diag_kill_qdss_threads(void)
{
	...
	if (remote_mask) {
		...
		if (qdss_mask & DIAG_CON_MPSS) {
			diag_send_cmds_to_peripheral_kill(MDM, DIAG_MODEM_PROC);
			qdss_mask ^= DIAG_CON_MPSS;
		}
		if (qdss_mask & DIAG_CON_LPASS) {
			diag_send_cmds_to_peripheral_kill(MDM, DIAG_LPASS_PROC);
			qdss_mask ^= DIAG_CON_LPASS;
		}
		if (qdss_mask & DIAG_CON_WCNSS) {
			diag_send_cmds_to_peripheral_kill(MDM, DIAG_WCNSS_PROC);
			qdss_mask ^= DIAG_CON_WCNSS;
		}
		if (qdss_mask & DIAG_CON_SENSORS) {
			diag_send_cmds_to_peripheral_kill(MDM, DIAG_SENSORS_PROC);
			qdss_mask ^= DIAG_CON_SENSORS;
		}
		if (qdss_mask & DIAG_CON_APSS) {
			diag_send_cmds_to_peripheral_kill(MDM, DIAG_APPS_PROC);
			qdss_mask ^= DIAG_CON_APSS;
		}
