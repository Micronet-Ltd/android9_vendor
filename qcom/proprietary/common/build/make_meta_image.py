...
class APPS_META:
    ...
    def generate_bootloader_radio_images(self):
        # get binary paths
        try:
            fblist = subprocess.check_output([self.mscript, 'get_file_vars', 'attr="fastboot"'])
            fbclist = subprocess.check_output([self.mscript, 'get_file_vars', 'attr="fastboot_complete"'])
