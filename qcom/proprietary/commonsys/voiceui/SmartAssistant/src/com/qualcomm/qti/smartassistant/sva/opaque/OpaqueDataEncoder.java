...
public class OpaqueDataEncoder {
    private final String TAG = OpaqueDataEncoder.class.getSimpleName();
    ...
    */
    ...
    public OpaqueDataEncoder(String smName, boolean hasHistBufferConfigParam,
        ...
        if (size > 0) {
            ...
            if (mHasConfidenceLevelsParam) {
                fillConfidenceLevelParam(buffer, 0, gmmKeyphraseLevel,
                        gmmUserLevel, cnnKeyphraseLevel, vopUserLevel,extraArray);
    ...
    private void fillConfidenceLevelParam(byte[] buffer, int startPos,
        ...
        if (mHasGMM) {
            ...
            for (int ii = 0; ii < keyphraseCount; ii++) {
                int userCount = mSmInfo.keywordInfo[ii].activeUsers.length;
                ...
                SoundTrigger.ConfidenceLevel[] confidenceLevels = extraArray[ii].confidenceLevels;
                // fill the ii user level
                for (int jj = 0; jj < userCount; jj++) {
                    pos = pos + 4 + jj * SIZE_OF_ST_USER_LEVEL;

                    // fill user id
                    fillInt(buffer, pos, confidenceLevels[jj].userId);
    ...
    private ListenTypes.SVASoundModelInfo query(String smFullName) {
        ...
        String filePath = FileUtils.APP_PATH + "/" + smFullName;
