...
public class DuerVoiceSdkImpl implements IVoiceSdk {
    ...
    public DuerVoiceSdkImpl(Context context,BaseAudioRecorder audioRecorder){
        mContext = context;
        ...
        appLauncherListener = new AppLauncherDeviceModule.IAppLauncherListener() {
            ...
            public void onLaunchApp(LaunchAppPayload payload) {
                if (!TextUtils.isEmpty(payload.getDeepLink())) {
                    try {
                        Uri uri = Uri.parse(payload.getDeepLink());
                        Intent intent = new Intent(Intent.ACTION_VIEW, uri);
                        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        mContext.startActivity(intent);
                    } catch (Exception e) {
                        e.printStackTrace();
                        Toast.makeText(mContext, mContext.getText(R.string.app_not_exists_msg),
                                Toast.LENGTH_SHORT).show();
                    }
                } else if (!TextUtils.isEmpty(payload.getPackageName())) {
                    boolean isLaunched = appLauncherDeviceModule.getAppLauncher().
                            launchAppByPackageName(mContext, payload.getPackageName());
                    if (!isLaunched){
                        Toast.makeText(mContext, mContext.getText(R.string.app_not_exists_msg),
                                Toast.LENGTH_SHORT).show();
                    }
                    ...
                    boolean isLaunched = appLauncherDeviceModule.getAppLauncher().
                            launchAppByName(mContext, payload.getAppName());
                    if (!isLaunched){
                        Toast.makeText(mContext, mContext.getText(R.string.app_not_exists_msg),
                                Toast.LENGTH_SHORT).show();
                    }
    ...
    private void initAlarmListener() {
        alarmListener = new AlarmsDeviceModule.IAlarmDirectiveListener() {
            ...
            public void onSetAlarmDirectiveReceived(SetAlarmPayload setAlarmPayload) {
                LogUtils.d(TAG, "Alarm : onSetAlarmDirectiveReceived");
