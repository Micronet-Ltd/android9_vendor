...
/**
 * \def QPLP_DEBUG
 * Log priority for DEBUG statements.
 *
 * Log priority for DEBUG statements.
 */
#define QPLP_DEBUG          2
...
/**
 * \def QPLP_CRITICAL
 * Log priority for critical messages.
 *
 * Log priority for critical messages.
 */
#define QPLP_CRITICAL     16
/**
 * \def QPLP_CONTROLLOGS
 * Log priority for CRITICAL + file redirection
 *
 * Log priority for CRITICAL + file redirection
 */
...
/**
 * \def QPLP_MINIMAL
 * Log priority for minmal logging messages.
 *
 * Log priority for minimal logging messages.
 */

#define QPLP_MINIMAL        32
/**
 * \def QPLP_ERROR
 * Log priority for errors.
 *
 * Log priority for errors.
 */
...
/**
 * \def QPLP_LOGPACKET
 * Log priority for log packet messages.
 *
 * Log priority for log packet messages.
 */
...
/**
 * \struct QpLogEvent
 * Event structure to be passed to qpLogEvent functions.
 *
 * This structure is passed to qpLogEvent to process a log.
 */
struct QpLogEvent {
...
{
  unsigned char   version;         /*Log packet version; range is 0 to 255*/
...
#define IMS_COMMON1(priority,f) \
        if(priority & logPriorityValue) {\
          if (bInit_Success) {\
            ...
            MSG_2(MSG_SSID_IMS, MSG_LEGACY_HIGH,str_cat([VTLib] %d %d ,f), getpid(),gettid());\
            pthread_mutex_unlock(&log_write_mutex);}\
          if (adbLogPropertyValue)\
            __android_log_print(ANDROID_LOG_ERROR, "VTLib", f); }
#define IMS_COMMON2(priority,f,a1) \
        if(priority & logPriorityValue) {\
          if (bInit_Success) {\
            pthread_mutex_lock(&log_write_mutex);\
            MSG_3(MSG_SSID_IMS, MSG_LEGACY_HIGH,str_cat([VTLib] %d %d ,f), getpid(),gettid(),(intptr_t)a1);\
            pthread_mutex_unlock(&log_write_mutex);}\
          if (adbLogPropertyValue)\
            __android_log_print(ANDROID_LOG_ERROR, "VTLib", f,a1); }
#define IMS_COMMON3(priority,f,a1,a2) \
        if(priority & logPriorityValue) {\
          if (bInit_Success) {\
            pthread_mutex_lock(&log_write_mutex);\
            MSG_4(MSG_SSID_IMS, MSG_LEGACY_HIGH,str_cat([VTLib] %d %d ,f), getpid(),gettid(),(intptr_t)a1,(intptr_t)a2);\
            pthread_mutex_unlock(&log_write_mutex);}\
          if (adbLogPropertyValue)\
            __android_log_print(ANDROID_LOG_ERROR, "VTLib", f,a1,a2); }
#define IMS_COMMON4(priority,f,a1,a2,a3) \
        if(priority & logPriorityValue) {\
          if (bInit_Success) {\
            pthread_mutex_lock(&log_write_mutex);\
            MSG_5(MSG_SSID_IMS, MSG_LEGACY_HIGH,str_cat([VTLib] %d %d ,f), getpid(),gettid(),(intptr_t)a1,(intptr_t)a2,(intptr_t)a3);\
            pthread_mutex_unlock(&log_write_mutex);}\
          if (adbLogPropertyValue)\
            __android_log_print(ANDROID_LOG_ERROR, "VTLib", f,a1,a2,a3); }
#define IMS_COMMON5(priority,f,a1,a2,a3,a4) \
        if(priority & logPriorityValue) {\
          if (bInit_Success) {\
            pthread_mutex_lock(&log_write_mutex);\
            MSG_6(MSG_SSID_IMS, MSG_LEGACY_HIGH,str_cat([VTLib] %d %d ,f), getpid(),gettid(),a1,a2,a3,a4);\
            pthread_mutex_unlock(&log_write_mutex);}\
          if (adbLogPropertyValue)\
            __android_log_print(ANDROID_LOG_ERROR, "VTLib", f,a1,a2,a3,a4);}
#define IMS_COMMON6(priority,f,a1,a2,a3,a4,a5) \
        if(priority & logPriorityValue) {\
          if (bInit_Success) {\
            pthread_mutex_lock(&log_write_mutex);\
            MSG_7(MSG_SSID_IMS, MSG_LEGACY_HIGH,str_cat([VTLib] %d %d ,f), getpid(),gettid(),a1,a2,a3,a4,a5);\
            pthread_mutex_unlock(&log_write_mutex);}\
          if (adbLogPropertyValue)\
            __android_log_print(ANDROID_LOG_ERROR, "VTLib", f,a1,a2,a3,a4,a5); }
...
#define IMS_COMMON1_STR(priority,f) \
        if(priority & logPriorityValue) {\
          if (bInit_Success) {\
            pthread_mutex_lock(&log_write_mutex);\
            MSG_SPRINTF_2(MSG_SSID_IMS, MSG_LEGACY_HIGH,str_cat([VTLib] %d %d ,f), getpid(),gettid());\
            pthread_mutex_unlock(&log_write_mutex);}\
...
#define IMS_COMMON2_STR(priority,f,a1) \
        if(priority & logPriorityValue) {\
          if (bInit_Success) {\
            pthread_mutex_lock(&log_write_mutex);\
            MSG_SPRINTF_3(MSG_SSID_IMS, MSG_LEGACY_HIGH,str_cat([VTLib] %d %d ,f), getpid(),gettid(),a1);\
            pthread_mutex_unlock(&log_write_mutex);}\
          if (adbLogPropertyValue && QPLP_MINIMAL != priority)\
            __android_log_print(ANDROID_LOG_ERROR, "VTLib", f,a1); }
#define IMS_COMMON3_STR(priority,f,a1,a2) \
        if(priority & logPriorityValue) {\
          if (bInit_Success) {\
            pthread_mutex_lock(&log_write_mutex);\
            MSG_SPRINTF_4(MSG_SSID_IMS, MSG_LEGACY_HIGH,str_cat([VTLib] %d %d ,f), getpid(),gettid(),a1,a2);\
            pthread_mutex_unlock(&log_write_mutex);}\
          if (adbLogPropertyValue && QPLP_MINIMAL != priority)\
            __android_log_print(ANDROID_LOG_ERROR, "VTLib", f,a1,a2); }
#define IMS_COMMON4_STR(priority,f,a1,a2,a3) \
        if(priority & logPriorityValue) {\
          if (bInit_Success) {\
            pthread_mutex_lock(&log_write_mutex);\
            MSG_SPRINTF_5(MSG_SSID_IMS, MSG_LEGACY_HIGH,str_cat([VTLib] %d %d ,f), getpid(),gettid(),a1,a2,a3);\
            pthread_mutex_unlock(&log_write_mutex);}\
          if (adbLogPropertyValue && QPLP_MINIMAL != priority)\
            __android_log_print(ANDROID_LOG_ERROR, "VTLib", f,a1,a2,a3); }
#define IMS_COMMON5_STR(priority,f,a1,a2,a3,a4) \
        if(priority & logPriorityValue) {\
          if (bInit_Success) {\
            pthread_mutex_lock(&log_write_mutex);\
            MSG_SPRINTF_6(MSG_SSID_IMS, MSG_LEGACY_HIGH,str_cat([VTLib] %d %d ,f), getpid(),gettid(),a1,a2,a3,a4);\
            pthread_mutex_unlock(&log_write_mutex);}\
          if (adbLogPropertyValue && QPLP_MINIMAL != priority)\
            __android_log_print(ANDROID_LOG_ERROR, "VTLib", f,a1,a2,a3,a4); }
...
#define IMS_COMMON6_STR(priority,f,a1,a2,a3,a4,a5) \
        if(priority & logPriorityValue) {\
          if (bInit_Success) {\
            pthread_mutex_lock(&log_write_mutex);\
            MSG_SPRINTF_7(MSG_SSID_IMS, MSG_LEGACY_HIGH,str_cat([VTLib] %d %d ,f), getpid(),gettid(),a1,a2,a3,a4,a5);\
            pthread_mutex_unlock(&log_write_mutex);}\
          if (adbLogPropertyValue && QPLP_MINIMAL != priority)\
            __android_log_print(ANDROID_LOG_ERROR, "VTLib", f,a1,a2,a3,a4,a5); }
...
#define QP_IMPORT_C
#define QP_EXPORT_C
QP_IMPORT_C int getModemTimeProperty();
...
QP_IMPORT_C uint64_t getCurrentAVTime();
QP_IMPORT_C int getAVTimerNode();
QP_IMPORT_C uint64_t getModemTime();
QP_IMPORT_C void setModemTime(uint64_t modem_time);
QP_IMPORT_C void setAppsTime(uint64_t apps_time);
QP_IMPORT_C uint64_t getAppsTime();
...
QP_IMPORT_C uint8_t skipFrame(uint64_t currenttstamp, uint64_t previouststamp,
                              uint32_t framerate);
QP_IMPORT_C void qpLogChangePriority(int priority);
QP_IMPORT_C void qpLogEventRecord(uint16_t evt, const char* pkt,
                                  uint32_t length);

QP_IMPORT_C void qpLogPacket(int priority, uint16_t logcode, void *buf,
                             uint32_t length);

QP_IMPORT_C void qpControlLogging(uint32_t fileNo, uint32_t lineNo, int32 arg1,
...
#define _QPLOG_PACKET(priority, logcode,buf,length)          \
      qpLogPacket(priority, logcode, buf, length);
...
#define ERROR1(f)                       if(1) {\
                                          ...
                                          if(bInit_Success) {\
                                            pthread_mutex_lock(&log_write_mutex);\
                                            MSG_SPRINTF_2(MSG_SSID_IMS, MSG_LEGACY_ERROR,str_cat([VTLib] %d %d ,f), getpid(),gettid());\
                                            pthread_mutex_unlock(&log_write_mutex); }  }
#define ERROR2(f,a1)                    if(1) {\
                                          __android_log_print(ANDROID_LOG_ERROR, "VTLib", f,a1);\
                                          if(bInit_Success) {\
                                            pthread_mutex_lock(&log_write_mutex);\
                                            MSG_SPRINTF_3(MSG_SSID_IMS, MSG_LEGACY_ERROR,str_cat([VTLib] %d %d ,f), getpid(),gettid(),a1);\
                                            pthread_mutex_unlock(&log_write_mutex); } }
#define ERROR3(f,a1,a2)                 if(1) {\
                                          __android_log_print(ANDROID_LOG_ERROR, "VTLib", f,a1,a2);\
                                          if(bInit_Success) {\
                                            pthread_mutex_lock(&log_write_mutex);\
                                            MSG_SPRINTF_4(MSG_SSID_IMS, MSG_LEGACY_ERROR,str_cat([VTLib] %d %d ,f), getpid(),gettid(),a1,a2);\
                                            pthread_mutex_unlock(&log_write_mutex); } }
#define ERROR4(f,a1,a2,a3)              if(1) {\
                                          __android_log_print(ANDROID_LOG_ERROR, "VTLib", f,a1,a2,a3);\
                                          if(bInit_Success) {\
                                            pthread_mutex_lock(&log_write_mutex);\
                                            MSG_SPRINTF_5(MSG_SSID_IMS, MSG_LEGACY_ERROR,str_cat([VTLib] %d %d ,f), getpid(),gettid(),a1,a2,a3);\
                                            pthread_mutex_unlock(&log_write_mutex); } }
#define ERROR5(f,a1,a2,a3,a4)           if(1) {\
                                          __android_log_print(ANDROID_LOG_ERROR, "VTLib", f,a1,a2,a3,a4);\
                                          if(bInit_Success) {\
                                            pthread_mutex_lock(&log_write_mutex);\
                                            MSG_SPRINTF_6(MSG_SSID_IMS, MSG_LEGACY_ERROR,str_cat([VTLib] %d %d ,f), getpid(),gettid(),a1,a2,a3,a4);\
                                            pthread_mutex_unlock(&log_write_mutex); } }
#define ERROR6(f,a1,a2,a3,a4,a5)        if(1) {\
                                          __android_log_print(ANDROID_LOG_ERROR, "VTLib", f,a1,a2,a3,a4,a5);\
                                          if(bInit_Success) {\
                                            pthread_mutex_lock(&log_write_mutex);\
                                            MSG_SPRINTF_7(MSG_SSID_IMS, MSG_LEGACY_ERROR,str_cat([VTLib] %d %d ,f), getpid(),gettid(),a1,a2,a3,a4,a5); \
                                            pthread_mutex_unlock(&log_write_mutex); } }
...
*/
#define LOGPACKET(a1,a2,a3)              _QPLOG_PACKET(QPLP_LOGPACKET,a1,a2,a3)
