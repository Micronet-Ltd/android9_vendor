  ...
  do {
    ...
    if (m_codecConfig.eCodec != VIDEO_CODEC_H263) {
      ...
      if (m_codecConfig.iOut_Width != (int32_t)iWidth &&
        ...
        /* Always set parameters in LANDSCAPE mode. This is a requirement from
           Video team. LANDSCAPE means, width is always larger than height */
...
QPE_VIDEO_ERROR VideoCodecEncoder::GetEncodedFrame()
{
  ...
  if (eVideoStateActive == iRecorderState && mCodec) {
    ...
    if (status >= 0) {
      ...
      if (info.flags & 0x01) {
        m_ullLastIdrTime =
          currentAppsTS; // update the iframe time for encoder generated iframe as well.
      }
      ...
      ERROR2("GetEncodedFrame: unexpected info code: %ld", status);
  ...
  switch (m_codecConfig.eCodec) {
    ...
    case VIDEO_CODEC_H264:
      ...
      if (m_codecConfig.eH264ProfileType == VIDEO_AVC_PROFILE_BASELINE) {
        ...
        CRITICAL2("Configure: H264 profile set to 0x%x", VT_AVCProfileCBPProfile);
      } else {
...
QPE_VIDEO_ERROR VideoCodecEncoder::AdaptVideoIdrPeriodicity(int32_t iFrameInt)
{
  ...
  if (0 == iFrameInt) {
    nPFrames = 0;
    nIDRPeriod = 1;
  } else if (MAX_IDR_INTERVAL == iFrameInt) {
