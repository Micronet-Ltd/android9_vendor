...
import com.android.backup.ContactsContract.CommonDataKinds.Relation;
...
import com.qualcomm.qti.backup_wrapper.PhoneNumberUtilsWrapper;
...
public class VCardBuilder {
    ...
    public VCardBuilder(final int vcardType, boolean pim) { // Moidyf
                                                            // liuhongxing
                                                            // 20110106
        ...
        if (mIsDoCoMo) {
            mCharsetString = SHIFT_JIS;
            // Do not use mCharsetString bellow since it is different from
            // "SHIFT_JIS" but
            // may be "DOCOMO_SHIFT_JIS" or something like that (internal
            // expression used in
    ...
    private boolean containsNonEmptyName(final ContentValues contentValues) {
        ...
        return !(TextUtils.isEmpty(familyName) && TextUtils.isEmpty(middleName) &&
                ...
                TextUtils.isEmpty(phoneticMiddleName) && TextUtils.isEmpty(phoneticGivenName) && TextUtils
                    .isEmpty(displayName));
    }
    ...
    private ContentValues getPrimaryContentValue(final List<ContentValues> contentValuesList) {
        ...
        for (ContentValues contentValues : contentValuesList) {
            ...
            if (isSuperPrimary != null && isSuperPrimary > 0) {
                ...
                if (isPrimary != null && isPrimary > 0 &&
                    ...
                    primaryContentValues = contentValues;
                    // Do not break, since there may be ContentValues with
                    // "super primary"
                    // afterword.
        ...
        if (primaryContentValues == null) {
            if (subprimaryContentValues != null) {
                // We choose the first ContentValues if any "primary"
                // ContentValues does not exist.
    ...
    /**
     * For safety, we'll emit just one value around StructuredName, as external
     * importers may get confused with multiple "N", "FN", etc. properties,
     * though it is valid in vCard spec.
     */
    public VCardBuilder appendNameProperties(final List<ContentValues> contentValuesList) {
        ...
        if (!TextUtils.isEmpty(familyName) || !TextUtils.isEmpty(givenName)) {
            ...
            if (mIsDoCoMo) {
                Log.e(LOG_TAG, "appendNameProperties mIsDoCoMo: " + mIsDoCoMo);
                ...
                // DoCoMo phones require that all the elements in the
                // "family name" field.
                ...
                if (!mIsPim) { // Modified by liuhongxing 20110224
                    mBuilder.append(VCARD_ITEM_SEPARATOR);
            ...
            // FN property
            if (!mIsPim) { // Modified by liuhongxing 20110106
            ...
            // Note: "CHARSET" param is not allowed in vCard 3.0, but we may add
            // it
            // when it would be useful for external importers, assuming no
            // external
    ...
    private void appendPhoneticNameFields(final ContentValues contentValues) {
        ...
        if (mIsV30) {
            ...
            //
            // Also, DoCoMo's specification requires vCard composer to use just
            // the first
            // column.
            // i.e.
            ...
            for (ContentValues contentValues : contentValuesList) {
                ...
                if (mIsPim) {
                    ...
                    for (String actualPhoneNumber : phoneNumberList)
                    {
                        if (!phoneSet.contains(actualPhoneNumber)) {
                            ...
                            try {
                                PhoneNumberUtilsWrapper.formatNumber(tmpBuilder.toString(), format);
                            } catch (WrapperNotSupportException wrapperE) {
                                Log.e(LOG_TAG, wrapperE.getMessage());
                            }
                ...
                if (type == Phone.TYPE_PAGER) {
                    ...
                    if (!phoneSet.contains(phoneNumber)) {
                        // zhangqiang modify for exporting same phonenumber
                        // phoneSet.add(phoneNumber);
                        appendTelLine(type, label, phoneNumber, isPrimary);
                    }
                } else {
                    // The entry "may" have several phone numbers when the
                    // contact entry is
                    // corrupted because of its original source.
                    //
                    // e.g. I encountered the entry like the following.
                    // "111-222-3333 (Miami)\n444-555-6666 (Broward; 305-653-6796 (Miami); ..."
                    // This kind of entry is not able to be inserted via Android
                    // devices, but
                    ...
                    for (String actualPhoneNumber : phoneNumberList) {
                        if (!phoneSet.contains(actualPhoneNumber)) {
                            ...
                            final int format = FORMAT_JAPAN;
                            try {
                                final String formattedPhoneNumber =
                                        PhoneNumberUtilsWrapper.formatNumber(actualPhoneNumber, format);
                                // zhangqiang modify for exporting same phonenumber
    ...
    private List<String> splitIfSeveralPhoneNumbersExist(final String phoneNumber) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            // TODO: add a test case for string with '+', and care the other
            // possible issues
            // which may happen by ignoring non-digits other than '+'.
            if (Character.isDigit(ch) || ch == '+' || ch == 'P' || ch == 'p' || ch == 'W'
                    || ch == 'w' || ch == '*' || ch == '#') {
                builder.append(ch);
            } else if ((ch == ';' || ch == '\n') && builder.length() > 0) {
    ...
    private PostalStruct tryConstructPostalStruct(ContentValues contentValues) {
        ...
        if (!VCardUtils.areAllEmpty(rawAddressArray)) {
            ...
            // This looks inefficient since we encode rawLocality and
            // rawNeighborhood twice,
            ...
            // We use the second value ("Extended Address") just because
            // Japanese mobile phones
            // do so. If the other importer expects the value be in the other
            // field, some flag may
            // be needed.
            final StringBuffer addressBuffer = new StringBuffer();
            addressBuffer.append(VCARD_ITEM_SEPARATOR);
    ...
    public VCardBuilder appendWebsites(final List<ContentValues> contentValuesList) {
        ...
        if (contentValuesList != null) {
            for (ContentValues contentValues : contentValuesList) {
                ...
                // Note: vCard 3.0 does not allow any parameter addition toward
                // "URL"
    ...
    public VCardBuilder appendOrganizations(final List<ContentValues> contentValuesList) {
        ...
        if (contentValuesList != null) {
            for (ContentValues contentValues : contentValuesList) {
                ...
                // final String orgline = orgBuilder.toString();
                boolean needcharset = !(VCardUtils.containsOnlyPrintableAscii(company) &&
                        VCardUtils.containsOnlyPrintableAscii(department));
                boolean needQuotedPrintable = mShouldUseQuotedPrintable &&
                        !VCardUtils.containsOnlyNonCrLfPrintableAscii(company) &&
                        !VCardUtils.containsOnlyNonCrLfPrintableAscii(department);
                appendLine(VCardConstants.PROPERTY_ORG, valuelist, needcharset, needQuotedPrintable);
    ...
    public VCardBuilder appendNotes(final List<ContentValues> contentValuesList) {
        ...
        if (contentValuesList != null) {
            if (mOnlyOneNoteFieldIsAvailable) {
                ...
                // This means we scan noteStr completely twice, which is
                // redundant.
            ...
            for (final ContentValues contentValues : contentValuesList) {
                ...
                if (eventType == Event.TYPE_BIRTHDAY) {
                    ...
                    final boolean isPrimary = (isPrimaryAsInteger != null ?
                            (isPrimaryAsInteger > 0) : false);
                    if (isPrimary) {
                        // We don't break here since "super primary" birthday
                        // may exist later.
                        primaryBirthday = birthdayCandidate;
                    ...
                    // Event types other than Birthday is not supported by
                    // vCard.
    ...
    public void appendPostalLine(final int type, final String label,
        ...
        switch (type) {
            ...
            case StructuredPostal.TYPE_CUSTOM: {
                if (!TextUtils.isEmpty(label)
                    ...
                    // Just for safety, we add "X-" at the beggining of each
                    // label.
        ...
        if (appendCharset) {
            // Strictly, vCard 3.0 does not allow exporters to emit charset
            // information,
            // but we will add it since the information should be useful for
            // importers,
            //
            // Assume no parser does not emit error with this parameter in vCard
            // 3.0.
            mBuilder.append(VCARD_PARAM_SEPARATOR);
            mBuilder.append(mVCardCharsetParameter);
        }
    ...
    public void appendTelLine(final Integer typeAsInteger, final String label,
        ...
        switch (type) {
            ...
            case Phone.TYPE_PAGER: {
                if (mIsDoCoMo) {
                    // Not sure about the reason, but previous implementation
                    // had
            ...
            case Phone.TYPE_WORK_MOBILE: {
                parameterList.addAll(
                        Arrays.asList(VCardConstants.PARAM_TYPE_WORK,
                                VCardConstants.PARAM_TYPE_CELL));
                break;
            }
            ...
            case Phone.TYPE_CUSTOM: {
                if (TextUtils.isEmpty(label)) {
                    ...
                    if (VCardUtils.isValidInV21ButUnknownToContactsPhoteType(upperLabel)) {
                        ...
                        // Note: Strictly, vCard 2.1 does not allow "X-"
                        // parameter without
                        // "TYPE=" string.
                        parameterList.add("X-" + label);
                    }
    ...
    public void appendAndroidSpecificProperty(final String mimeType, ContentValues contentValues) {
        ...
        for (String rawValue : rawValueList) {
            ...
            if (reallyUseQuotedPrintable) {
                ...
                // TODO: one line may be too huge, which may be invalid in vCard
                // 3.0
    ...
    private void appendTypeParameters(final List<String> types) {
        // We may have to make this comma separated form like "TYPE=DOM,WORK" in
        // the future,
        // which would be recommended way in vcard 3.0 though not valid in vCard
        // 2.1.
        boolean first = true;
        for (final String typeValue : types) {
            // Note: vCard 3.0 specifies the different type of acceptable type
            // Strings, but
            // we don't emit that kind of vCard 3.0 specific type since there
            // should be
            // high probabilyty in which external importers cannot understand
            // them.
            //
            // e.g. TYPE="\u578B\u306B\u3087" (vCard 3.0 allows non-Ascii
            // characters if they
            // are quoted.)
    ...
    private void appendTypeParameter(final StringBuilder builder, final String type) {
        // Refrain from using appendType() so that "TYPE=" is not be appended
        // when the
        // device is DoCoMo's (just for safety).
        //
        // Note: In vCard 3.0, Type strings also can be like this:
        // "TYPE=HOME,PREF"
     ...
     * Strictly, adding charset information is invalid in VCard 3.0. However
     * we'll add the info only when charset we use is not UTF-8 in vCard 3.0
     * format, since parser side may be able to use the charset via this field,
     * though we may encounter another problem by adding it. e.g. Japanese
     * mobile phones use Shift_Jis while RFC 2426 recommends UTF-8. By adding
     * this field, parsers may be able to know this text is NOT UTF-8 but
     * Shift_Jis.
     */
    ...
    /**
     * Append '\' to the characters which should be escaped. The character set
     * is different not only between vCard 2.1 and vCard 3.0 but also among each
     * device. Note that Quoted-Printable string must not be input here.
     */
