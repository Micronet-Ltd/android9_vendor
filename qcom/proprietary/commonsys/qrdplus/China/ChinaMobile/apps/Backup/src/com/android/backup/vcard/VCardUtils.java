...
import com.qualcomm.qti.backup_wrapper.ContactsWrapper;
import com.qualcomm.qti.backup_wrapper.TextUtilsWrapper;
...
public class VCardUtils {
    ...
    static {
        ...
        // \u643A\u5E2F\u96FB\u8A71 = Full-width Hiragana "Keitai-Denwa" (mobile
        // phone)
        // \u643A\u5E2F = Full-width Hiragana "Keitai" (mobile phone)
        // \u30B1\u30A4\u30BF\u30A4 = Full-width Katakana "Keitai" (mobile
        // phone)
        // \uFF79\uFF72\uFF80\uFF72 = Half-width Katakana "Keitai" (mobile
        // phone)
    ...
    public static Object getPhoneTypeFromStrings(Collection<String> types,
        ...
        if (types != null) {
            for (String typeString : types) {
                ...
                Log.d("VCardUtils", "getPhoneTypeFromStrings typeString: " + typeString);
                if (typeString.equals(VCardConstants.PARAM_TYPE_PREF)) {
                    hasPref = true;
                }
                if (typeString.equals(VCardConstants.PARAM_TYPE_FAX)) {
                    isFax = true;
                } else if (typeString.equals(VCardConstants.PARAM_TYPE_CELL)) {
                    isCell = true;
                } else if (typeString.equals(VCardConstants.PARAM_TYPE_VOICE)) {
                    isVoice = true;
                } else {
        ...
        if (type < 0) {
            if (hasPref) {
                ...
                type = sKnownPhoneTypeMap_StoI.get(VCardConstants.PARAM_TYPE_CELL);
            } else {
    ...
    public static boolean isMobilePhoneLabel(final String label) {
        ...
        // Detail: Until Donut, there isn't TYPE_MOBILE for email while there is
        // now.
        // To support mobile type at that time, this custom label had been used.
        return (ContactsWrapper.ContactMethodsColumns.MOBILE_EMAIL_TYPE_NAME
                .equals(label)
     ...
     * conversion may be performed in this method.
     */
    public static void insertStructuredPostalDataUsingContactsStruct(int vcardType,
            ...
            int myId) {// sxm add ID 20121121
        builder.withValueBackReference(StructuredPostal.RAW_CONTACT_ID, myId);
    ...
    public static boolean containsOnlyPrintableAscii(final Collection<String> values) {
        ...
        for (final String value : values) {
            ...
            try {
                if (!TextUtilsWrapper.isPrintableAsciiOnly(value)) {
                    return false;
                }
    ...
    /**
     * <P>
     * Returns true when the given String is categorized as "word" specified in
     * vCard spec 2.1.
     * </P>
     * <P>
    ...
    /**
     * Guesses the format of input image. Currently just the first few bytes are
     * used. The type "GIF", "PNG", or "JPEG" is returned when possible. Returns
     * null when the guess failed.
     *
    ...
    public static String guessImageType(final byte[] input) {
        ...
        if (input.length >= 3 && input[0] == 'G' && input[1] == 'I' && input[2] == 'F') {
            ...
            // Note: vCard 2.1 officially does not support PNG, but we may have
            // it and
