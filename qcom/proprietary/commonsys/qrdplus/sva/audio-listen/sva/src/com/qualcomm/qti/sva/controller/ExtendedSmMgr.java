...
public class ExtendedSmMgr {
    ...
    public void addSoundModel(Context context, String smFullName) {
        ...
        if (ISmModel.MODEL_VERSION.VERSION_3_0 == soundModel.getSoundModelVersion()) {
            LogUtils.d(TAG, "addSoundModel: 3.0 model set opaque data transfer enabled");
                ...
                if (IExtendedSmModel.VERSION_0300 == modelInfo.version) {
                    ...
                    */
                    for (String userItem : modelInfo.userNames) {
                        // adapt the STManager API limitation
                        if (Global.getIsLegacySTModuleSessionImpl(context)) {
    ...
    public ByteBuffer getLanguageModel() {
        ...
        String filePath = Global.PATH_ROOT + "/" + Global.NAME_LANGUAGE_MODEL;
        if (FileUtils.isExist(filePath)) {
            try {
