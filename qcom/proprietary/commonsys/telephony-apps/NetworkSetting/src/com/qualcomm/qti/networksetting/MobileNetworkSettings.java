...
public class MobileNetworkSettings extends Activity  {
    ...
    public static class MobileNetworkFragment extends PreferenceFragment implements
        ...
        public static final int NT_GSM_CDMA_EVDO = 24;//add by Zemel 20170613 for bug 14709

        // Number of active Subscriptions to show tabs
        private static final int TAB_THRESHOLD = 2;
        /* add by hudayu for bug 0016549 */
        private static final int SINGLE_CARD_SLOT = 1;
        ...
        private static final String CARRIER_MODE_CUCC = "cucc";
        //Carrier mode
        ...
        private boolean isFromLovdream=false;
        ...
        private boolean mShowSimpleNetworkType;
        ...
        public void onCreate(Bundle icicle) {
            ...
            if("com.lovdream.TestList".equals(from)){
                isFromLovdream =true;
            }
            ...
            mShowSimpleNetworkType = getResources().getBoolean(R.bool.show_simple_network_type) && !isFromLovdream;
        ...
        /**
         *add by Zemel 20170509 for bug 15016
         */
        private void updateListPreference(ListPreference listPreference){
            Log.d(LOG_TAG,"updateListPreference");
            Resources r = getResources();
            /* add by Zemel 20170613 for bug 14709 start */
            if(mShowSimpleNetworkType){
                listPreference.setSummary(r.getString(R.string.preferred_network_4G));
                listPreference.setValueIndex(0);
                listPreference.setEntries(r.getStringArray(R.array.preferred_network_mode_choices_less));
                if (mPhone != null && mPhone.getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA){
                    listPreference.setEntryValues(r.getStringArray(R.array.preferred_network_mode_values_less_ct));
                }else{
                    listPreference.setEntryValues(r.getStringArray(R.array.preferred_network_mode_values_less));
                }
                return;
            }
            /* add by Zemel 20170613 for bug 14709 end */
            if(CARRIER_MODE_CUCC.equals(mCarrierMode)){
                listPreference.setSummary(r.getString(R.string.preferred_network_mode_lte_gsm_wcdma_summary));
                listPreference.setEntries(r.getStringArray(R.array.preferred_network_mode_choices_cucc));
                listPreference.setEntryValues(r.getStringArray(R.array.preferred_network_mode_values_cucc));
            }
        }
        ...
        private void updateBodyAdvancedFields(Activity activity, PreferenceScreen prefSet,
            ...
            if(mShowSimpleNetworkType){
                /* add by hudayu for bug 0016549 */
                if (phoneId == currentPrimarySlot||SINGLE_CARD_SLOT == mTelephonyManager.getPhoneCount()){
                    updateListPreference(mButtonPreferredNetworkMode);
                }
            ...
            /*modify by Zemel 20170721 for bug 15454 start*/
            boolean isSecondCard = false;
            /* add by hudayu for bug 0016549 */
            if(SINGLE_CARD_SLOT < mTelephonyManager.getPhoneCount()){
                isSecondCard = mShowSimpleNetworkType && phoneId != currentPrimarySlot;
            }

            mButtonPreferredNetworkMode.setEnabled(hasActiveSubscriptions && !isSecondCard);
            if (isSecondCard){
                UpdatePreferredNetworkModeSummary(NT_GSM_CDMA_EVDO);
            }
            /*modify by Zemel 20170721 for bug 15454 end*/
        ...
        private int getPreferredNetworkModeForSubId() {
            ...
            if (DBG) log("getPreferredNetworkModeForSubId: phoneNwMode = " + nwMode +
                    " subId = "+ subId);
            /*add by Zemel 20170509 for bug 15016 start*/
            CharSequence[] values = mButtonPreferredNetworkMode.getEntryValues();
            if(!Arrays.asList(values).contains(Integer.toString(nwMode))){
                if(CARRIER_MODE_CUCC.equals(mCarrierMode)){
                    nwMode = Phone.NT_MODE_LTE_GSM_WCDMA;
                }
            }
            android.util.Log.d(LOG_TAG, "nwMode--->"+nwMode);
            /*add by Zemel 20170509 for bug 15016 end*/
            return nwMode;
        }
		...
		if(mShowSimpleNetworkType){
            ...
            final int phoneId = mPhone.getPhoneId();
            final int currentPrimarySlot = Settings.Global.getInt(mPhone.getContext().getContentResolver(),
                    CONFIG_CURRENT_PRIMARY_SUB,
                    SubscriptionManager.INVALID_SIM_SLOT_INDEX);
            /* add by hudayu for bug 0016549 */
            if (phoneId != currentPrimarySlot&&SINGLE_CARD_SLOT < mTelephonyManager.getPhoneCount()) {
                NetworkMode = NT_GSM_CDMA_EVDO;
            }
			switch (NetworkMode){
				case Phone.NT_MODE_LTE_TDSCDMA_CDMA_EVDO_GSM_WCDMA:
					mButtonPreferredNetworkMode.setSummary(R.string.preferred_network_4G);
					mButtonPreferredNetworkMode.setValueIndex(0);
					break;
				case Phone.NT_MODE_TDSCDMA_CDMA_EVDO_GSM_WCDMA:
					mButtonPreferredNetworkMode.setSummary(R.string.preferred_network_3G);
					mButtonPreferredNetworkMode.setValueIndex(1);
					break;
				case NT_GSM_CDMA_EVDO:
                case Phone.NT_MODE_GSM_ONLY:
                case Phone.NT_MODE_CDMA_NO_EVDO:
					mButtonPreferredNetworkMode.setSummary(R.string.preferred_network_2G);
					mButtonPreferredNetworkMode.setValueIndex(2);
					break;
				default :
					mButtonPreferredNetworkMode.setSummary(R.string.preferred_network_4G);
					mButtonPreferredNetworkMode.setValueIndex(0);
