...
public class ImsServiceSub extends MmTelFeature
    ...
    private void handleSrvccResponse(AsyncResult ar) {
        if (mTracker != null && ar.exception == null) {
            mTracker.calculateOverallSrvccState((int[]) ar.result);
        } else {
    ...
    public void changeEnabledCapabilities(CapabilityChangeRequest request,
        ...
        for (CapabilityPair cp : capsToEnable) {
            sendChangeCapabilityRequest(cp, true);
        }
        for (CapabilityPair cp : capsToDisable) {
            sendChangeCapabilityRequest(cp, false);
        }
    }

    private void sendChangeCapabilityRequest(CapabilityPair cp, boolean enable) {
        int feature = getFeature(cp.getCapability(), cp.getRadioTech());
        int rat = getNetworkTypeFromRadioTech(cp.getRadioTech());
        if (feature == INVALID_FEATURE_VALUE || rat == INVALID_RAT_VALUE) {
            loge("changeEnabledCapabilities :: Invalid CapabilityPair feature="
                    + feature + " rat=" + rat);
            return;
        }
        mImsConfigImpl.setCapabilityValue(feature, rat, enable ? ImsConfig.FeatureValueConstants.ON
                : ImsConfig.FeatureValueConstants.OFF, new SetCapabilityListener());
    }
    ...
    /**
     * Utility method to map ImsRegistrationBaseImpl values for RAT.
     *
     * @param radioTech Has one of the following values:
     * {@link ImsRegistrationImplBase#REGISTRATION_TECH_LTE} or
     * {@link ImsRegistrationImplBase#REGISTRATION_TECH_IWLAN}
     *
     * @return The appropriate TelephonyManager NETWORK_TYPE value, if a mapping
     *         exists, else -1.
     */
    private static int getNetworkTypeFromRadioTech(
            @ImsRegistrationImplBase.ImsRegistrationTech int radioTech) {
        int network = INVALID_RAT_VALUE;
        switch (radioTech) {
    ...
    private void onGetPacketErrorCountDone(IQtiImsExtListener listener, AsyncResult ar) {
        if (listener != null) {
            try {
                ...
                loge("onGetPacketErrorCountDone " + t);
            }
        } else {
    ...
    private void onSetCallForwardTimerDone(IQtiImsExtListener listener, AsyncResult ar) {
        ...
        if (listener != null) {
            if (ar.result != null) {
                ...
                String failCause = response.getFailureCause();
                if (!failCause.isEmpty()) {
                    loge("onSetCallForwardTimerDone Failure cause: " +
                            failCause);
                    try {
