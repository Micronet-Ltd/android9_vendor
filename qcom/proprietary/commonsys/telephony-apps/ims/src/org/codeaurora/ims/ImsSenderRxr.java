...
public class ImsSenderRxr extends ImsPhoneBaseCommands {
     ...
     *
     */
    class ImsRadioResponse extends vendor.qti.hardware.radio.ims.V1_4.IImsRadioResponse.Stub {
      ...
      *
      */
    class ImsRadioIndication extends vendor.qti.hardware.radio.ims.V1_4.IImsRadioIndication.Stub {
        ...
        public void onIncomingCallAutoRejected(AutoCallRejectionInfo rejectInfo) {
            ...
            dc.callFailCause = new ImsReasonInfo(
                    ImsRadioUtilsV13.getImsReasonForCallFailCause(rejectInfo.autoRejectionCause),
                    rejectInfo.sipErrorCode);
            dc.callFailCause.mCode = dc.callFailCause.mCode == ImsReasonInfo.CODE_UNSPECIFIED ?
                    ImsReasonInfo.CODE_REJECT_UNKNOWN : dc.callFailCause.mCode;
        ...
        public void onSupplementaryServiceIndication(StkCcUnsolSsResult ss) {
            ...
            if (ssData.isTypeCf() && ssData.isTypeInterrogation()) {
                ...
                if (ret instanceof ImsCallForwardTimerInfo[]) {
                    ...
                    ImsCallForwardInfo[] cfInfo = new ImsCallForwardInfo[num];
                    for (int i = 0; i < num; i++) {
                        vendor.qti.hardware.radio.ims.V1_0.CallForwardInfo rCfInfo = cfData.
                                cfInfo.get(i);
                        cfInfo[i] = new ImsCallForwardInfo(ImsRadioUtils.
                                getUTInterfaceCFReasonFromCommandsInterfaceCFReason(rCfInfo.reason),
                                rCfInfo.status,
                                rCfInfo.toa,
                        ...
                        Log.i(this, "[SS Data] CF Info " + i + " : " +  cfInfo[i]);
                    }
                    ssData.setCallForwardingInfo(cfInfo);
                }
            } else if (ssData.isTypeIcb() && ssData.isTypeInterrogation()) {
                num = ss.cbNumInfo.size();
                ImsSsInfo[] imsSsInfo = new ImsSsInfo[num];
                for (int i = 0; i < num; i++) {
                    imsSsInfo[i] = new ImsSsInfo(ss.cbNumInfo.get(i).status,
                            ss.cbNumInfo.get(i).number);
                    Log.i(this, "[SS Data] ICB Info " + i + " : " +  imsSsInfo[i]);
                }
                ssData.setImsSpecificSuppServiceInfo(imsSsInfo);
            } else {
                SsInfoData ssInfoData = ss.ssInfoData.get(0);
                num = ssInfoData.ssInfo.size();
                int[] ssInfo = new int[num];
                for (int i = 0; i < num; i++) {
                    ssInfo[i] = ssInfoData.ssInfo.get(i);
                    Log.i(this, "[SS Data] SS Info " + i + " : " + ssInfo[i]);
                }
                ssData.setSuppServiceInfo(ssInfo);
            }
    ...
    private boolean sendErrorOnImsRadioDown(IFRequest rr, String msgIdString) {
        ...
        if (imsRadioV10() == null) {
            ...
            rr.onError(ImsErrorCode.RADIO_NOT_AVAILABLE, null);
            rr.release();
            return true;
        }
        return false;
    ...
    public void setServiceStatus(Message result, int srvType, int network, int enabled,
        ...
        try {
            ...
            imsRadioV10().setServiceStatus(rr.mSerial, serviceStatusInfo);
        } catch (Exception ex) {
            removeFromQueueAndSendResponse(rr.mSerial);
    ...
    public void sendConfigRequest(int requestType, int item, boolean boolValue,
        ...
        try {
            ...
            if (requestType == MessageId.REQUEST_GET_IMS_CONFIG) {
                ...
                imsRadioV10().setConfig(rr.mSerial, configInfo);
            }
        } catch (Exception ex) {
            removeFromQueueAndSendResponse(rr.mSerial);
            Log.e(this, msgIdString + " request to IImsRadio: Exception: " + ex);
