  ...
  (JNIEnv * /*env*/, jclass /*cls*/)
{
  ...
  rv = svc_sock_terminate(SVC_SOCKET_NAME);
  LOGD("terminate %d", rv);
  ...
  (JNIEnv * /*env*/, jclass /*cls*/, jint display, jint doPause)
{
  ...
  if (display == 1) {
    dpy = IDisplayConfig::DisplayType::DISPLAY_EXTERNAL;
    ALOGD("pauseExternal: HDMI: %d", doPause);
  } else if (display == 2) {
    dpy = IDisplayConfig::DisplayType::DISPLAY_VIRTUAL;
  ...
  if (doPause) {
    rv = client->setSecondayDisplayStatus(dpy,
