...
WebHttpServer::WebHttpServer(unsigned int httpPort, Host& host)
    :m_port(httpPort), m_host(host)
{
    m_pCommandsExecutor = make_shared<CommandsExecutor>(host);
    ...
    m_pServer->config.port = static_cast<unsigned short>(httpPort);
