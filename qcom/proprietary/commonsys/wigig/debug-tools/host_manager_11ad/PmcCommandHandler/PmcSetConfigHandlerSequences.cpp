...
OperationStatus PmcSetConfigHandlerSequences::ConfigureUCodeEvents(const std::string& deviceName, bool ucodeCoreWriteEnabled)
{
    ...
    if (ucodeCoreWriteEnabled)
    {
        ...
        auto st2 = PmcService::GetInstance().GetPmcRegistersAccessor().WritePmcRegister(deviceName, "MAC_RGF.MAC_SXD.LOCAL_REGISTER_IF.LOCAL_WR_REG.sxd_local_wr_data", value);
        if (!st2.IsSuccess())
        {
            return st2;
        }
        // 2. Configure MSXD ranges in PMC_UCODE_WR_RANGE_0
        auto st3 = PmcService::GetInstance().GetPmcRegistersAccessor().WritePmcRegister(deviceName,
                "MAC_EXT_RGF.MAC_SXD_EXT.PMC_UCODE_WR_RANGE.MAC_EXT_MAC_EXT_RGF_MAC_SXD_EXT_PMC_UCODE_WR_RANGE_0.sxd_pmc_start_1", 1U);
        if (!st3.IsSuccess())
        {
            return st3;
        }
        auto st4 = PmcService::GetInstance().GetPmcRegistersAccessor().WritePmcRegister(deviceName,
                "MAC_EXT_RGF.MAC_SXD_EXT.PMC_UCODE_WR_RANGE.MAC_EXT_MAC_EXT_RGF_MAC_SXD_EXT_PMC_UCODE_WR_RANGE_0.sxd_pmc_end_1", 1U);
        if (!st4.IsSuccess())
        {
            return st4;
        }
        auto st5 = PmcService::GetInstance().GetPmcRegistersAccessor().WritePmcRegister(deviceName,
                "MAC_EXT_RGF.MAC_SXD_EXT.PMC_UCODE_WR_RANGE.MAC_EXT_MAC_EXT_RGF_MAC_SXD_EXT_PMC_UCODE_WR_RANGE_0.sxd_pmc_start_2", 1U);
        if (!st5.IsSuccess())
        {
            return st5;
        }
        auto st6 = PmcService::GetInstance().GetPmcRegistersAccessor().WritePmcRegister(deviceName,
                "MAC_EXT_RGF.MAC_SXD_EXT.PMC_UCODE_WR_RANGE.MAC_EXT_MAC_EXT_RGF_MAC_SXD_EXT_PMC_UCODE_WR_RANGE_0.sxd_pmc_end_2", 1U);
        if (!st6.IsSuccess())
        {
            return st6;
        }
