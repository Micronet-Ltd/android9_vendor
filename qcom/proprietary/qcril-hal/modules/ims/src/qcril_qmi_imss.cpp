...
ims_settings_uri_line_type_v01 converToQmiMultiIdentityLineType(int lineType) {
    switch(lineType) {
        case QcrilImsMultiIdentityInfo::LINE_TYPE_PRIMARY:
...
)
{
  ...
  do
  {
    auto& lines = *(static_cast<std::vector<QcrilImsMultiIdentityInfo>*>(params_ptr->data));
    ...
    {
        auto& in_line = lines[i];
        auto& out_line = qmi_req.line_info_list[i];
        ...
        QCRIL_LOG_DEBUG("MultiIdentity line[%d] : msisdn = %s , line type = %d", i,
...
)
{
   ...
   if (NULL != params_ptr)
   {
      auto resp = static_cast<ims_settings_set_digits_req_rsp_msg_v01*>(params_ptr->data);
  ...
  if ( qcril_reqlist_query_by_req_id( req_id, &instance_id, &req_info ) == E_SUCCESS )
  {
    ...
    else
    {
      ...
      switch(msg_id)
      {
        ...
        case QMI_IMS_SETTINGS_SET_DIGITS_RSP_V01:
          qcril_qmi_imss_register_multi_identity_lines_resp_hdlr(&req_data);
          break;
