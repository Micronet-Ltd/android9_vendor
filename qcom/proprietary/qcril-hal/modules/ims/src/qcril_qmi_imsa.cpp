...
void qcril_qmi_imsa_cleanup()
{
   ...
   qcril_qmi_imsa_store_service_status_cl(
       FALSE, IMSA_NO_SERVICE_V01,  // SMS SERVICE_STATUS
...
void qcril_qmi_imsa_voice_info_ind_hdlr(void *ind_data_ptr) {
    ...
    if (NULL != ind_msg_ptr) {
        ...
        ims_VoiceInfo voiceInfo = qcril_qmi_voice_info_to_ims(ind_msg_ptr->voice_info);
        imsRadioSendMessage(0, ims_MsgType_UNSOL_RESPONSE,
                                 ims_MsgId_UNSOL_VOICE_INFO,
                                 ims_Error_E_SUCCESS,
                                 &voiceInfo,
                                 sizeof(ims_VoiceInfo));
    } else {
...
void qcril_qmi_imsa_multi_identity_registration_status_change_ind_hdlr(void *ind_data_ptr) {
    ...
    if (registration_info && registration_info->line_info_list_valid) {
        std::vector<QcrilImsMultiIdentityInfo> lines_info;
        for (uint32_t i =0; i < registration_info->line_info_list_len; i++) {
            auto& qmi_line_info = registration_info->line_info_list[i];
            QcrilImsMultiIdentityInfo line_info(qmi_line_info.msisdn,
        ...
        if ( params_ptr->data )
        {
            do
            {
                ...
                if (ims_req->has_addressInfo)
                {
                    ...
                    if (ims_req->addressInfo.street.arg)
                    {
                        req_msg.street_valid = TRUE;
                        strlcpy((char*)req_msg.street, (char*)ims_req->addressInfo.street.arg,
                                sizeof(req_msg.street));
                        QCRIL_LOG_DEBUG("Street: %s", ims_req->addressInfo.street.arg);
                    }

                    if (ims_req->addressInfo.houseNumber.arg)
                    {
                        req_msg.housenumber_valid = TRUE;
                        strlcpy((char*)req_msg.housenumber,
                                (char*)ims_req->addressInfo.houseNumber.arg,
                                sizeof(req_msg.housenumber));
                        QCRIL_LOG_DEBUG("House Number: %s", ims_req->addressInfo.houseNumber.arg);
                    }
                }
...
)
{
    ...
    else if(!qcril_qmi_imsa_info.ims_srv_status.sms_service_status_valid)
    {
        ...
        std::shared_ptr<SmsImsServiceStatusInd::SmsImsServiceStatusInfo> get_sms_ims_service_status_info_msg = nullptr;
        Message::Callback::Status apiStatus;
        apiStatus = msg->dispatchSync(get_sms_ims_service_status_info_msg);

        std::ostringstream ss;
        ss << "qcril_qmi_imsa_store_service_status_cl status = " << (int) apiStatus;
        Log::getInstance().d(ss.str());

        if ((apiStatus != Message::Callback::Status::SUCCESS) ||
            !get_sms_ims_service_status_info_msg)
        {
            Log::getInstance().d("failed to get sms over ims status"
                + std::to_string((int) apiStatus));
        }
        else
        {
            if(SmsImsServiceStatusInd::SmsImsServiceStatus::NO_SERVICE == get_sms_ims_service_status_info_msg->sms_ims_service_status)
            {
                sms_service_status = IMSA_NO_SERVICE_V01;
            }
            else if(SmsImsServiceStatusInd::SmsImsServiceStatus::LIMITED_SERVICE == get_sms_ims_service_status_info_msg->sms_ims_service_status)
            {
                sms_service_status = IMSA_LIMITED_SERVICE_V01;
            }
            else if(SmsImsServiceStatusInd::SmsImsServiceStatus::FULL_SERVICE == get_sms_ims_service_status_info_msg->sms_ims_service_status)
            {
...
)
{
  ...
  if( NULL != params_ptr && NULL != params_ptr->data )
  {
    auto resp_msg_ptr = static_cast<imsa_get_virtual_line_rsp_msg_v01*>(params_ptr->data);
    if (QMI_RESULT_SUCCESS_V01 == resp_msg_ptr->resp.result &&
        resp_msg_ptr->virtual_line_info_valid)
    {
      ...
      {
        for (auto& p_associated_uri : resp_msg_ptr->virtual_line_info.uri_list)
        {
          virtual_lines.push_back(std::string(p_associated_uri.uri));
        }
        VirtualLineInfo info(msisdn, virtual_lines);
        imsRadioSendMessage(params_ptr->t, ims_MsgType_RESPONSE,
            ims_MsgId_REQUEST_QUERY_VIRTUAL_LINE_INFO,
            ims_Error_E_SUCCESS, &info, sizeof(info));
      }
    }
    else {
      QCRIL_LOG_INFO("error: %d", resp_msg_ptr->resp.error);
      imsRadioSendMessage(params_ptr->t, ims_MsgType_RESPONSE,
          ims_MsgId_REQUEST_QUERY_VIRTUAL_LINE_INFO, ims_Error_E_GENERIC_FAILURE, NULL, 0);
    }
  }
  else {
    QCRIL_LOG_ERROR("params_ptr or params_ptr->data is NULL");
...
)
{
  ...
  if (imsa_inited)
  {
    do
    {
      ...
      std::string& msisdn = *(static_cast<std::string*>(params_ptr->data));
      ...
      //We need to store the msisdn to map request and response
      reqlist_entry.req_data = (void*)msisdn.c_str();
      reqlist_entry.req_datalen = msisdn.length() + 1;
...
//=========================================================================
//  FUNCTION:   qcril_qmi_imsa_sms_ims_service_status_hdlr
//=========================================================================
