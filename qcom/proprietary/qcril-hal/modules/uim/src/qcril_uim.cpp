...
)
{
  ...
  if (req_data_ptr != NULL)
  {
    ...
    else if (req_data_ptr->req_type == UimRequestMsgData::UIM_REQUEST_GET_GID)
    {
      std::shared_ptr<UimGetGIDRequestMsg>  req_msg_ptr =
          std::static_pointer_cast<UimGetGIDRequestMsg>(req_data_ptr->msg_ptr);

      if (req_msg_ptr != NULL)
      {
        std::shared_ptr<std::vector<uint8_t>> gid_ptr = nullptr;

        if (rsp_ptr != nullptr && rsp_len != 0)
        {
          std::vector<uint8_t> gid_data((uint8_t *)rsp_ptr, (uint8_t *)rsp_ptr+rsp_len);

          gid_ptr = std::make_shared<std::vector<uint8_t>>(gid_data);
          if (gid_ptr != nullptr)
          {
            memcpy(gid_ptr.get(), rsp_ptr, rsp_len);
          }
        }
        req_msg_ptr->sendResponse(req_msg_ptr, Message::Callback::Status::SUCCESS, gid_ptr);
      }
    }
    else
    {
...
)
{
  ...
  QCRIL_LOG_QMI( modem_id, "qmi_uim_service", "get slot status" );
...
)
{
  ...
  switch(callback_params_ptr->qmi_rsp_data.rsp_id)
  {
    case QMI_UIM_SRVC_READ_TRANSPARENT_RSP_MSG:
      ...
      else if (orig_ptr->request_id == UIM_INTERNAL_REQUEST_GET_GID)
      {
        qcril_uim_get_gid_resp(callback_params_ptr);
      }
      else
      {
