...
void UimModule::UimProcessQmiCommandCallback(std::shared_ptr<UimQmiUimResponseMsg> uim_qmi_resp_msg_ptr)
{
  ...
  {
    std::unique_lock<std::mutex> lock(mMsgListLock);
...
void UimModule::UimProcessGetImsiReq(std::shared_ptr<UimGetImsiRequestMsg> imsi_req_ptr)
{
  ...
  if (mReady)
  {
    ...
    if (imsi_ptr != nullptr)
    {
      ...
      {
        strlcpy(imsi_rsp_ptr.get(), imsi_ptr, strlen(imsi_ptr) + 1);
      }
    }
  }
...
void UimModule::UimProcessGetGIDReq(std::shared_ptr<UimGetGIDRequestMsg> req_ptr)
{
  ...
  qcril_request_params_type                 params{};
  qcril_request_return_type                 ret_params{};
  qcril_uim_gid_req_type                    gid_req;
  ...
  msg_data_ptr = new UimRequestMsgData{req_ptr, UimRequestMsgData::UIM_REQUEST_GET_GID, nullptr};
  ...
  gid_req.slot_id = req_ptr->get_slot();
  gid_req.aid_ptr = req_ptr->get_aid();
  gid_req.gid_type = (qcril_uim_gid_type)req_ptr->get_gid_type();
  gid_req.app_type = req_ptr->get_app_type();

  params.instance_id = qmi_ril_get_process_instance_id();
  params.modem_id    = QCRIL_DEFAULT_MODEM_ID;
  params.datalen     = sizeof(gid_req);
  params.data        = &gid_req;
  params.t           = (RIL_Token)msg_data_ptr;
  params.event_id_android = UIM_INTERNAL_REQUEST_GET_GID;
  qcril_uim_request_get_gid(&params, &ret_params);
...
)
{
      ...
      rsp_data_ptr != nullptr)
  {
    ...
    /* Sync message going to the card, release the lock, lock for async message */
