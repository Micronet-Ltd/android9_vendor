...
static void secureElementClearChannelIdList(std::vector<uint8_t> &ch_id_list)
{
  for (auto ch_id:ch_id_list)
  {
    ...
    (void) qcril_uim_send_qmi_sync_msg(QCRIL_UIM_REQUEST_LOGICAL_CHANNEL,
                                       &close_ch,
                                       &rsp_data);
  }
  ch_id_list.clear();
...
)
{
  ...
  qmi_err_code = qcril_uim_send_qmi_sync_msg(QCRIL_UIM_REQUEST_OPEN_LOGICAL_CHANNEL,
                                             &open_ch,
                                             &rsp_data);
...
)
{
      ...
      ((data.data()[3] & 0x03) == QCRIL_UIM_P2_MASK_SELECT_NEXT))
  {
    ...
    qmi_err_code = qcril_uim_send_qmi_sync_msg(QCRIL_UIM_REQUEST_RESELECT,
                                               &reselect_params,
                                               &rsp_data);
  ...
  if (not_select_next)
  {
    ...
    qmi_err_code = qcril_uim_send_qmi_sync_msg(QCRIL_UIM_REQUEST_SEND_APDU,
                                               &apdu_params,
                                               &rsp_data);
...
)
{
  ...
  qmi_err_code = qcril_uim_send_qmi_sync_msg(QCRIL_UIM_REQUEST_LOGICAL_CHANNEL,
                                             &close_ch,
                                             &rsp_data);
...
{
  QmiUimSetupRequestCallback qmiUimSetupCallback("secure-element-Config-Module-Token");
  android::status_t          ret = android::OK;

  (void)msg;

  ModemEndPointFactory<UimModemEndPoint>::getInstance().buildEndPoint()->requestSetup(
                                  "Client-secure-element-Serv-Token", &qmiUimSetupCallback);
