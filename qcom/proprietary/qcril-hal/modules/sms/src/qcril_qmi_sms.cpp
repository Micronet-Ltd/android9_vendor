...
#define QCRIL_IMS_SMS_SOURCE                "persist.vendor.radio.ims_sms_from_ims_hidl"
...
static int qmi_ril_ims_sms_from_ims_pipe = 0;
...
)
{
  ...
  else
  {
     ...
     memset(args, '\0', PROPERTY_VALUE_MAX);
     QCRIL_SNPRINTF( property_name, sizeof( property_name ), "%s", QCRIL_IMS_SMS_SOURCE );
     property_get( property_name, args, "" );

     if( strcmp(args, "false") )
     {
        qmi_ril_ims_sms_from_ims_pipe = 1;
     }
     else
     {
        qmi_ril_ims_sms_from_ims_pipe = 0;
     }

     QCRIL_LOG_DEBUG( "Is IMS SMS from IMS PIPE = %d", qmi_ril_ims_sms_from_ims_pipe );
  }
...
)
{
  ...
  else if (raw_send_resp->resp.result == QMI_RESULT_FAILURE_V01)
  {
             ...
             (raw_send_resp->resp.error == QMI_ERR_INVALID_OPERATION_V01))
    {
      retry_status = TRUE;
            ...
            (params_ptr->event_id_android == RIL_REQUEST_IMS_SEND_SMS))
    {
  ...
  if (params_ptr->event_id == QCRIL_EVT_IMS_SOCKET_REQ_SEND_SMS)
  {
    ...
    QcrilImsSmsResponseData ims_sms_resp = {static_cast<uint32_t>(send_msg_response.messageRef),
            ImsSmsSendStatusResult::SEND_STATUS_OK,
            ImsSmsSendFailureReason::RESULT_ERROR_NONE};
    qcril_sms_convert_to_ims_errors(ril_err, raw_send_resp, tech,
            ims_sms_resp.smsStatusResult, ims_sms_resp.reason);
    QCRIL_LOG_INFO("status result %d, reason %d", ims_sms_resp.smsStatusResult, ims_sms_resp.reason);
    imsRadioSendMessage(params_ptr->t, ims_MsgType_RESPONSE, ims_MsgId_REQUEST_SEND_SMS,
            ims_Error_E_SUCCESS, (void*)&ims_sms_resp, sizeof(ims_sms_resp));
  }
  else
  {
    if (ril_err == RIL_E_SUCCESS)
...
{
  auto ack_resp = static_cast<wms_send_ack_resp_msg_v01*>(params_ptr->data);

  if(ack_resp->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    QCRIL_LOG_INFO("QMI_WMS_SEND_ACK_RESP received: SUCCESS");
    /* will handle both IMS and CS case */
    qcril_send_empty_payload_request_response(QCRIL_DEFAULT_INSTANCE_ID,
            params_ptr->t, params_ptr->event_id, RIL_E_SUCCESS);
  }
  else
  {
    auto ril_err = qcril_qmi_util_convert_qmi_response_codes_to_ril_result(
        QMI_NO_ERR, &(ack_resp->resp));
...
)
{
  ...
  if (res == RIL_E_SUCCESS)
  {
    ...
    ims_registration_state[0] = ims_sms_state.is_ims_wms_reg;
    ims_registration_state[1] = (ims_sms_state.format == QMI_RIL_SMS_FORMAT_3GPP2) ?
...
)
{
   ...
   do
   {
      ...
      if ((params_ptr->event_id != QCRIL_EVT_IMS_SOCKET_REQ_SEND_SMS) &&
            (TRUE == qcril_qmi_sms_block_retry_3pgg_sms_on_lte_only(ims_sms_msg->tech, ims_sms_msg->retry)))
      {
...
#if 1
         if (cdma_sms_msg)
            dumpRILCdmaFormat(*cdma_sms_msg);
#endif
      }
      else
      {
      ...
      qcril_reqlist_default_entry( params_ptr->t, params_ptr->event_id,
                                   ...
                                   QCRIL_REQ_AWAITING_CALLBACK,
                                   QCRIL_EVT_NONE, NULL, &reqlist_entry );
      reqlist_entry.req_data = &ims_sms_msg->tech;
      reqlist_entry.req_datalen = sizeof(ims_sms_msg->tech);
      if ( (reqlist_status = qcril_reqlist_new( instance_id, &reqlist_entry )) != E_SUCCESS )
      {
  ...
  if(ril_err != RIL_E_SUCCESS)
  {
    if (params_ptr->event_id == QCRIL_EVT_IMS_SOCKET_REQ_SEND_SMS)
    {
      QcrilImsSmsResponseData ims_sms_resp = {static_cast<uint32_t>(-1),
...
)
{
   ...
   do {
    ...
    auto ack_data = static_cast<const QcrilImsSmsAckData*>(params_ptr->data);
    ...
    qcril_reqlist_default_entry( params_ptr->t, params_ptr->event_id,
                                 ...
                                 QCRIL_REQ_AWAITING_CALLBACK,
                                 QCRIL_EVT_NONE, NULL, &reqlist_entry );
    auto reqlist_status = qcril_reqlist_new(instance_id, &reqlist_entry);
    if (reqlist_status != E_SUCCESS)
    {
      QCRIL_LOG_ERROR("Failed to add the reqlist entry for IMS SMS acknowledgement");
      ril_err = qcril_qmi_map_internalerr_from_reqlist_new_to_ril_err(reqlist_status);
      break;
    }
    ...
    request_msg.ack_information.success =
        (ack_data->smsDeliverResult == ImsSmsDeliverStatusResult::DELIVER_STATUS_OK) ?
        0x01 : 0x00;
    if (request_msg.ack_information.success != 0x01)
    {
        // TODO: fill error cause when detailed error cause is defined for IMS
    }

    uint32_t user_data = QCRIL_COMPOSE_USER_DATA( instance_id, QCRIL_DEFAULT_MODEM_ID, reqlist_entry.req_id );
    if (qmi_client_sms_send_async(QMI_WMS_SEND_ACK_REQ_V01,
                                  &request_msg,
                                  sizeof(wms_send_ack_req_msg_v01),
                                  sizeof(wms_send_ack_resp_msg_v01),
...
/*===========================================================================
  ...
  FUNCTION:  qcril_sms_request_ims_ack_sms_report

===========================================================================*/
/*!
    @brief
    Handles ims_MsgId_REQUEST_ACK_SMS_REPORT

    @return
    None.
*/
/*=========================================================================*/
void qcril_sms_request_ims_ack_sms_report
(
   const qcril_request_params_type *const params_ptr,
   qcril_request_return_type *const ret_ptr /*!< Output parameter */
)
...
)
{
  ...
  if ( qcril_reqlist_query_by_req_id( req_id, &instance_id, &req_info ) == E_SUCCESS )
  {
    ...
    switch (msg_id)
    {
      ...
      {
        RIL_RadioTechnologyFamily tech = RADIO_TECH_FAMILY_UNKNOWN;
        if (NULL != req_info.req_data &&
                sizeof(RIL_RadioTechnologyFamily) == req_info.req_datalen)
            tech = *static_cast<RIL_RadioTechnologyFamily*>(req_info.req_data);
        qcril_mo_sms_cb(&req_data, tech);
        break;
      }
    ...
    */
    ...
    if (is_processing_ok)
    {
      ...
      if (event_report_ind->sms_on_ims_valid && event_report_ind->sms_on_ims
            && qmi_ril_ims_sms_from_ims_pipe)
      {
        // TODO: confirm what the payload format for CDMA SMS is
        // From the interface, it is just vec<uint8_t> pdu.
        // Use RIL_CDMA_SMS_Message now
        std::vector<uint8_t> payload;
        (void)convertCdmaFormatToPseudoPdu(
            *(reinterpret_cast<RIL_CDMA_SMS_Message*>(buf_ptr)), payload);
        // implict vector to hidl_vec conversion
        IncomingImsSms incoming_sms = {"3gpp2", payload,
                VerificationStatus::STATUS_VALIDATION_NONE};
        imsRadioSendMessage(0, ims_MsgType_UNSOL_RESPONSE,
                ims_MsgId_UNSOL_INCOMING_SMS,
                ims_Error_E_SUCCESS, &incoming_sms, sizeof(incoming_sms));
      }
      else
      {
...
VerificationStatus qcril_sms_convert_qmi_verstat_to_hidl
(
...
)
{
    switch(verstat_status_info) {
        case MT_IMS_TN_VALIDATION_NONE_V02:
            return VerificationStatus::STATUS_VALIDATION_NONE;
        case MT_IMS_TN_VALIDATION_PASS_V02:
            return VerificationStatus::STATUS_VALIDATION_PASS;
        case MT_IMS_TN_VALIDATION_FAIL_V02:
            return VerificationStatus::STATUS_VALIDATION_FAIL;
        default:
  ...
  */
  ...
  if (is_processing_ok)
  {
    ...
    if (qcril_sms_check_if_gw_msg_is_status_report(&event_report_ind->transfer_route_mt_message, message_ref))
    {
      ...
      if (is_on_ims && qmi_ril_ims_sms_from_ims_pipe)
      {
        ImsSmsSendStatusReport status_report = {message_ref, "3gpp", buf_data};
        imsRadioSendMessage(0, ims_MsgType_UNSOL_RESPONSE,
              ims_MsgId_UNSOL_SMS_STATUS_REPORT,
              ims_Error_E_SUCCESS, &status_report, sizeof(status_report));
      }
      else
      {
        qcril_default_unsol_resp_params( instance_id,
              (int) RIL_UNSOL_RESPONSE_NEW_SMS_STATUS_REPORT, &unsol_resp );
        unsol_resp.resp_pkt = buf_data.data();
        unsol_resp.resp_len = buf_data.size();
        qcril_send_unsol_response( &unsol_resp );
      }
    }
    else
    {
      if (event_report_ind->sms_on_ims_valid && event_report_ind->sms_on_ims
            && qmi_ril_ims_sms_from_ims_pipe)
      {
        VerificationStatus verstat = VerificationStatus::STATUS_VALIDATION_NONE;
        if (event_report_ind->verstat_status_info_valid)
        {
            verstat = qcril_sms_convert_qmi_verstat_to_hidl(
                event_report_ind->verstat_status_info);
        }
        IncomingImsSms incoming_sms = {"3gpp", buf_data, verstat};
        imsRadioSendMessage(0, ims_MsgType_UNSOL_RESPONSE,
              ims_MsgId_UNSOL_INCOMING_SMS,
              ims_Error_E_SUCCESS, &incoming_sms, sizeof(incoming_sms));
      }
