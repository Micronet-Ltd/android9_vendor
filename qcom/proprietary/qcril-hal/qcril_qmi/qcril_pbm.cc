...
int qmi_ril_set_ecc_property(char* ecc_numbers)
{
    ...
    if (!no_space && pbm_cache_info.alt_emer_num_info_valid)
    {
        if (!qcril_other_is_number_found(pbm_cache_info.alt_emer_num_info.number, ecc_prop_val))
        {
            item_len = strlen(pbm_cache_info.alt_emer_num_info.number);
            ...
            else
            {
                ...
                strlcat(ecc_prop_val, pbm_cache_info.alt_emer_num_info.number, PROPERTY_VALUE_MAX);
            }
        }
    }
    PBM_CACHE_UNLOCK();
