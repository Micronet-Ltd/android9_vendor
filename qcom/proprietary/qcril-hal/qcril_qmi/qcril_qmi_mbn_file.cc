...
void qcril_qmi_mbn_diff_send_unsol_msg(int result, int index, xmlChar* id, xmlChar* ref_val, xmlChar* dev_val)
{
    ...
    do
    {
      ...
      qcril_hook_unsol_response( QCRIL_DEFAULT_INSTANCE_ID, QCRIL_EVT_HOOK_UNSOL_PDC_VALIDATE_CONFIGS,
                                static_cast<void *>(payload), sizeof(mcfg_diff_unsol_msg_type) + str_len);
      qcril_free(payload);
    } while (0);
}
...
)
{
  ...
  do
  {
    ...
    result = static_cast<RIL_Errno>(mcfg_get_qc_version(mbn_file_path, &qc_version, &qc_version_len));
  } while (0);
...
)
{
  ...
  do
  {
    ...
    memcpy(mbn_file_path, params_ptr->data, params_ptr->datalen);
    mbn_file_path[params_ptr->datalen] = 0;
    result = static_cast<RIL_Errno>(mcfg_get_oem_version(mbn_file_path, &oem_version, &oem_version_len));
  } while (0);
