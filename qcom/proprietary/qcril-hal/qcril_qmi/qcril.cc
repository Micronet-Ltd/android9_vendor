...
#define QCRIL_PRIMARY_MODEM_INFO_LOCK()   pthread_mutex_lock(&primary_modem_info.mdm_mutex);
#define QCRIL_PRIMARY_MODEM_INFO_UNLOCK() pthread_mutex_unlock(&primary_modem_info.mdm_mutex);

#define QCRIL_SECONDARY_MODEM_INFO_LOCK()   pthread_mutex_lock(&secondary_modem_info.mdm_mutex);
#define QCRIL_SECONDARY_MODEM_INFO_UNLOCK() pthread_mutex_unlock(&secondary_modem_info.mdm_mutex);
...
)
{
  ...
  pthread_mutexattr_init(&primary_modem_info.mdm_attr);
  pthread_mutex_init(&primary_modem_info.mdm_mutex, &primary_modem_info.mdm_attr);

  pthread_mutexattr_init(&secondary_modem_info.mdm_attr);
  pthread_mutex_init(&secondary_modem_info.mdm_mutex, &secondary_modem_info.mdm_attr);
...
uint32_t qmi_ril_fw_android_request_get_handling_capabilities( int android_request_id )
{
  ...
  switch ( android_request_id )
  {
    ...
    case RIL_REQUEST_ENABLE_5G_ONLY:
    case RIL_REQUEST_DISABLE_5G:
      res = QMI_RIL_FW_ANDROID_REQUEST_HNDL_CAPS_COMPOSE_CAPS( QMI_RIL_FW_ANDROID_REQUEST_HNDL_CAPS_ALLOW_MULTIPLE
                                                               | QMI_RIL_FW_ANDROID_REQUEST_HNDL_CAPS_UNBIND_DROP_OFF
                                                               | QMI_RIL_FW_ANDROID_REQUEST_HNDL_CAPS_LEGACY_DROP_OFF
                                                               | QMI_RIL_FW_ANDROID_REQUEST_HNDL_CAPS_IN_FAMILY_RING,
