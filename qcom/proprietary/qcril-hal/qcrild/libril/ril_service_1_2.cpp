...
Return<void> RadioImpl_1_2::setIndicationFilter_1_2(int32_t serial, ::android::hardware::hidl_bitfield<V1_2::IndicationFilter> indicationFilter){
    ...
    auto msg = std::make_shared<NasEnablePhysChanConfigReporting>((int32Filter & RIL_UR_PHYSICAL_CHANNEL_CONFIG) != 0, ctx,
...
void convertRilCellInfoToHal_1_2(V1_2::CellInfo &hidlCellInfo, RIL_CellInfo_v12 &rilCellInfo) {
        switch(rilCellInfo.cellInfoType) {
            case RIL_CELL_INFO_TYPE_GSM: {
                ...
                cellInfoGsm->cellIdentityGsm.operatorNames.alphaLong =
                    convertCharPtrToHidlString(rilCellInfo.CellInfo.gsm.cellIdentityGsm.operatorNames.alphaLong);
                cellInfoGsm->cellIdentityGsm.operatorNames.alphaShort =
                    convertCharPtrToHidlString(rilCellInfo.CellInfo.gsm.cellIdentityGsm.operatorNames.alphaShort);
                break;
            }
            ...
            case RIL_CELL_INFO_TYPE_WCDMA: {
                ...
                cellInfoWcdma->cellIdentityWcdma.operatorNames.alphaLong =
                    convertCharPtrToHidlString(rilCellInfo.CellInfo.wcdma.cellIdentityWcdma.operatorNames.alphaLong);
                cellInfoWcdma->cellIdentityWcdma.operatorNames.alphaShort =
                    convertCharPtrToHidlString(rilCellInfo.CellInfo.wcdma.cellIdentityWcdma.operatorNames.alphaShort);
                break;
            }
            ...
            case RIL_CELL_INFO_TYPE_CDMA: {
                ...
                cellInfoCdma->cellIdentityCdma.operatorNames.alphaLong =
                    convertCharPtrToHidlString(rilCellInfo.CellInfo.cdma.cellIdentityCdma.operatorNames.alphaLong);
                cellInfoCdma->cellIdentityCdma.operatorNames.alphaShort =
                    convertCharPtrToHidlString(rilCellInfo.CellInfo.cdma.cellIdentityCdma.operatorNames.alphaShort);
                break;
            }
            ...
            case RIL_CELL_INFO_TYPE_LTE: {
                ...
                cellInfoLte->cellIdentityLte.operatorNames.alphaLong =
                    convertCharPtrToHidlString(rilCellInfo.CellInfo.lte.cellIdentityLte.operatorNames.alphaLong);
                cellInfoLte->cellIdentityLte.operatorNames.alphaShort =
                    convertCharPtrToHidlString(rilCellInfo.CellInfo.lte.cellIdentityLte.operatorNames.alphaShort);
...
int RadioImpl_1_2::sendGetIccCardStatusResponse(int slotId,
                                   int responseType, int serial, RIL_Errno e,
                                   void *response, size_t responseLen) {
    auto rsp = mRadioResponseV1_2;
    if (rsp != NULL) {
        ...
        if ( convertGetIccCardStatusResponse(responseInfo, cardStatus.base,
                responseType,
                serial,
                e,
                response,
                responseLen) == 0)
        {
            cardStatus.physicalSlotId = p_cur->physical_slot_id;
            cardStatus.atr = convertCharPtrToHidlString(p_cur->atr);
            cardStatus.iccid = convertCharPtrToHidlString(p_cur->iccid);

            QCRIL_LOG_DEBUG("Sending V1_2::GetIccCardStatusResponse. physSlotId: %d",
                    cardStatus.physicalSlotId);
        } else {
            QCRIL_LOG_ERROR("Error converting GetIccCardStatusResponse");
        }
        Return<void> retStatus = rsp->
                  getIccCardStatusResponse_1_2(responseInfo, cardStatus);
        checkReturnStatus(retStatus);
    } else {
        RLOGE("sendGetIccCardStatusResponse: radioService[%d]->mRadioResponse_1_2 == NULL", slotId);
        RadioImpl::sendGetIccCardStatusResponse(slotId,
                responseType,
                serial,
                e,
                response,
...
int convertToHal(WcdmaSignalStrength &out, RIL_WCDMA_SignalStrength &in) {
    out.signalStrength = in.signalStrength;
    out.bitErrorRate = in.bitErrorRate;
    return 0;
}

int convertToHal(V1_2::WcdmaSignalStrength &out, RIL_WCDMA_SignalStrength &in) {
    int ret = 1;
...
hidl_vec<V1_2::PhysicalChannelConfig> toHal(const std::vector<NasPhysChanInfo> &info) {
...
int RadioImpl_1_2::sendCurrentPhysicalChannelConfigs(std::shared_ptr<NasPhysChanConfigMessage> msg) {
    ...
    if (rsp  && msg) {
        QCRIL_LOG_INFO("Sending current config. size: %d", msg->getInfo().size());
        Return<void> retStatus = rsp->currentPhysicalChannelConfigs(RadioIndicationType::UNSOLICITED,toHal(msg->getInfo()));
        checkReturnStatus(retStatus);

    } else {
        RLOGE("sendCurrentPhysicalChannelConfigs: radioService[%d]->mRadioResponse == NULL",
                mSlotId);
    }
...
class RilServiceModule: public Module {
    ...
    public:
        ...
        void handleNasPhysChanConfigMessage(std::shared_ptr<NasPhysChanConfigMessage> msg);
        void init() {
            Module::init();
...
RilServiceModule &get_RilServiceModule();


static load_module<RilServiceModule> the_module;

RilServiceModule &getRilServiceModule()
{
    return the_module.get_module();
}

RilServiceModule::RilServiceModule()
{
    ...
    mMessageHandler = {
        ...
        HANDLER(LinkCapIndMessage, RilServiceModule::handleLinkCapIndMessage),
        HANDLER(NasPhysChanConfigMessage, RilServiceModule::handleNasPhysChanConfigMessage),
    };
    QCRIL_LOG_INFO("Calling registerRadioImpl");
    RLOGD("Calling registerRadioImpl");
...
void RilServiceModule::handleLinkCapIndMessage(std::shared_ptr<LinkCapIndMessage> msg) {
    QCRIL_LOG_DEBUG("Handling LinkCapIndMessage %s",msg->dump().c_str());
    sp<RadioImpl_1_2> ri = radioImpl_1_2;
    if (ri) {
        pthread_rwlock_t *radioServiceRwlockPtr = radio::getRadioServiceRwlock(ri->mSlotId);
        int rwlockRet = pthread_rwlock_rdlock(radioServiceRwlockPtr);
        assert(rwlockRet == 0);

        ri->sendLinkCapInd(msg);

        rwlockRet = pthread_rwlock_unlock(radioServiceRwlockPtr);
        assert(rwlockRet == 0);
    }
}
void RilServiceModule::handleNasPhysChanConfigMessage(std::shared_ptr<NasPhysChanConfigMessage> msg) {
    QCRIL_LOG_DEBUG("Handling NasPhysChanConfigMessage %s",msg->dump().c_str());
    auto ri = radioImpl_1_2;
    if (ri) {
        QCRIL_LOG_DEBUG("Forwarding");
        pthread_rwlock_t *radioServiceRwlockPtr = radio::getRadioServiceRwlock(ri->mSlotId);
        int rwlockRet = pthread_rwlock_rdlock(radioServiceRwlockPtr);
        assert(rwlockRet == 0);

        ri->sendCurrentPhysicalChannelConfigs(msg);
