...
class QcrildServiceVersion
{
    ...
    public:
        QcrildServiceVersion(uint16_t major, uint16_t minor, uint16_t local = 0):
            major(major), minor(minor), local(local)
        { }

        bool operator< (const QcrildServiceVersion &other) const {
            QCRIL_LOG_DEBUG("major: %u, minor: %u local: %u", (unsigned)major, (unsigned)minor, (unsigned)local);
            QCRIL_LOG_DEBUG("o.major: %u, o.minor: %u o.local: %u", (unsigned)other.major, (unsigned)other.minor, (unsigned)other.local);
