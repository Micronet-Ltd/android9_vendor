    ...
    const ILocationProvider* providerSrc)
{
    ...
    do {
        ...
        if (report->mHasElapsedRealTimeInNanoSecs) {
            networkPos.valid_flag |= ULP_NETWORK_POSITION_HAS_ELAPSED_TIME;
            networkPos.elapsed_time = report->mElapsedRealTimeInNanoSecs;
        }
...
void IzatManager::handleStartGnssFix() {
    if (!mHandlingGnssFix && mGpsEnabled) {
        if (!mUlpStarted)
        {
        ...
        crit.provider_source = ULP_PROVIDER_SOURCE_HYBRID;
        crit.preferred_horizontal_accuracy = ULP_HORZ_ACCURACY_HIGH;
...
IzatLocation IzatManager::izatLocationFromUlpLocation(const UlpLocation *ulpLoc,
    ...
    if (NULL != locExtended) {
        ...
        if (locExtended->flags & GPS_LOCATION_EXTENDED_HAS_ELAPSED_TIME) {
            res.mHasElapsedRealTimeInNanoSecs = true;
            res.mElapsedRealTimeInNanoSecs = locExtended->elapsedTime;
        }
    ...
    for (; it != mDataItemList.end(); it++) {
        ...
        switch (dataItem->getId()) {
            ...
            case GPSSTATE_DATA_ITEM_ID: {
                ...
                mIzatManager->setGpsEnabled(gpsState->mEnabled);
                if (mIzatManager->getHandlingGnssFix() && !gpsState->mEnabled) {
