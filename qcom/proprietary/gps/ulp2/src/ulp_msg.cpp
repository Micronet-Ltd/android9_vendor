...
int ulp_msg_process_network_position_report (const UlpNetworkPositionReport* networkPositionPtr)
{
   ...
   do
   {
      ...
      if (networkPositionPtr->valid_flag & ULP_NETWORK_POSITION_HAS_ELAPSED_TIME) {
          location_extended.flags |=  GPS_LOCATION_EXTENDED_HAS_ELAPSED_TIME;
          location_extended.elapsedTime =
                  networkPositionPtr->elapsed_time;
      }
                                          ...
                                          const GpsLocationExtended& locationExtended)
{
   ...
   do
   {
      ...
      LOC_LOGV("Sending GNSS Pos to External DR, Lat: %f, Long: %f, RawdataSize: %d, RawData: %p",
               location.gpsLocation.latitude, location.gpsLocation.longitude,
               location.rawDataSize, (location.rawData)? location.rawData:0);
...
int ulp_msg_process_zpp_position_report (const enum loc_sess_status status,
                                         LocPosTechMask     tech_mask,
                                         const UlpLocation* locationPtr,
                                         const GpsLocationExtended* extendedLocationPtr)
{
   int ret_val = -1;
   ...
   do
   {
      ...
      ret_val = ulp_brain_process_zpp_position_report (status, tech_mask,
                                                       (UlpLocation*)locationPtr,
                                                       (GpsLocationExtended*)extendedLocationPtr);
