...
int ulp_data_remove_criteria (const UlpLocationCriteria* locationCriteriaPtr)
{
   ...
   for (i = 0; i < MAX_NUM_UNIQUE_CRITERIA; i++)
   {
      if (ulp_data.locationCriteriaArray[i].isUsed == true)
      {
         //If the remove criteria is for GPS provider then we want to simply remove it without
         //checking if it has identical criteria, as MSA requests get changed to singleshot
         //in ulp, where the remove criteria will still have periodic attribute. Also, there
         //will only be one request to ULP from GPS provider, as all the requests from app to
         //GPS provider will be consolidated into one.
         if ((ULP_PROVIDER_SOURCE_GNSS == locationCriteriaPtr->provider_source) &&
             (ULP_PROVIDER_SOURCE_GNSS ==
                      ulp_data.locationCriteriaArray[i].locationCriteria.provider_source))
         {
            LOC_LOGD ("%s, Remove Criteria is for GPS provider. mask: 0x%x\n", __func__,
                      locationCriteriaPtr->valid_mask);
            ulp_data.locationCriteriaArray[i].isUsed = false;
