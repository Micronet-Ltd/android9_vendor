...
#define PROCESS_NAME_XTWIFI_CLIENT "xtwifi-client"
...
int main()
{
    ...
    int result = 0;
    int soc_id = loc_read_device_soc_id();
    ...
    //special case to disable apps based on Soc Id
    if(soc_id > 0){
        for(i=0; i<proc_list_length;i++) {
          if((soc_id == SOC_ID_QM215)&&
             (strncmp(child_proc[i].name[0], PROCESS_NAME_XTWIFI_CLIENT,
                      strlen(PROCESS_NAME_XTWIFI_CLIENT)) == 0)) {
            LOC_LOGD("%s:%d]: disable process %s for soc_id %d",
                     __func__, __LINE__, i, child_proc[i].name[0], soc_id);
