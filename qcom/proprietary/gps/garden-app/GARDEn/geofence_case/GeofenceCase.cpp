...
void GeofenceCase::syncResumeGeofence(uint32_t geofenceId, int monitor_transitions) {
    gardenPrint("GEOFENCE RESUME -> id=%2u monitor_transitions=%d\n",
            geofenceId, monitor_transitions);
    {
        AutoLock lock(&mLock);
        setFlag(GEOFENCE_CASE_RESUME);
...
// and execute the coresponding command at the given time (relatively).
...
GARDEN_RESULT GeofenceCase::loadFromFileCase(std::string file) {
    ...
    std::string s;
    while(getline(in, s)) {
        const std::regex pt(".*(\\d{2}):(\\d{2}):(\\d{2}).(\\d{3}).*(geofence.*)\\x28(.*)\\x29");
        ...
        if (valid) {
            ...
            uint64_t time = (((atoi(hour.c_str()) * 60 + atoi(min.c_str())) * 60) +
                    atoi(sec.c_str())) * 1000 + atoi(msec.c_str());
            gardenPrint("%lld %s %s", time, func.c_str(), args.c_str());

            if (std::strstr(func.c_str(), "geofenceAdd") != nullptr) {
                std::regex pattern("(.*)[ ,](.*)[ ,](.*)[ ,](.*)[ ,](.*)[ ,](.*)[ ,](.*)[ ,](.*)");
                std:: match_results<std::string::const_iterator> param;
                valid = std::regex_match(args, param, pattern);
                if(valid) {
