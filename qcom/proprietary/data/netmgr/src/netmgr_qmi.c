...
#define NETMGR_MAX_IFACE     NETMGR_MAX_LINKS_PER_MODEM
...
int last_cleared_iface = NETMGR_MAX_IFACE;
/*---------------------------------------------------------------------------
        ...
        if (netmgr_main_get_qos_enabled())
        {
              ...
              ind_data->tech_name != WDS_TECHNOLOGY_NAME_MODEM_LINK_LOCAL_V01)
          {
            ...
            if (NETMGR_SUCCESS != netmgr_qmi_qos_cache_default_flow(link,
                ind_data->bearer_id_valid ? ind_data->bearer_id : NETMGR_BEARER_ID_INVALID,
...
)
{
  ...
  cmd->data.info.qos_flow.bearer_id = (NETMGR_QOS_EVENT_REPORT_FLOW_INFO_BEARER_ID_PARAM &
                                       flow_ptr->param_mask)?
                                       flow_ptr->bearer_id: NETMGR_BEARER_ID_INVALID;
...
)
{
  ...
  if (!netmgr_main_cfg.rtm_rmnet_enabled &&
      NETMGR_RMNET_SUCCESS != netmgr_rmnet_update_ul_aggregation(dev_name,
                                                                 agg_count,
                                                                 agg_size))
...
)
{
  ...
  for(i=0; i < netmgr_main_cfg.num_data_ports; ++i)
  {
         ...
         netmgr_qmi_cfg.links[i].wds_info.in_use == FALSE)
    {
      if((netmgr_qmi_port_in_call(i, modem_subs_id) == NETMGR_SUCCESS) &&
         ((last_cleared_iface < NETMGR_MAX_IFACE) && (last_cleared_iface != (int)i)))
        break;
      else
        continue;
    }
  ...
  {
    if(( last_cleared_iface < NETMGR_MAX_IFACE ) &&
