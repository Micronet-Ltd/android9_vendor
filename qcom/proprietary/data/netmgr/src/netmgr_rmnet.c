...
netmgr_rmnet_configure_ep_params()
{
             ...
             sizeof(NETMGR_PHYS_NET_DEV_RMNET_IPA)))
  {
    ...
    ds_get_epid(netmgr_main_cfg.phys_net_dev, &ep_type, &epid);
    netmgr_main_cfg.epid = epid;
    netmgr_log_med("%s: Not a IPA target. use static ep cofiguration epid %d",
                   __func__,netmgr_main_cfg.epid);
...
)
{
  ...
  mux_offset = NETMGR_REV_RMNET_START;
...
)
{
...
#ifdef FEATURE_DATA_IWLAN
  rev_mux_offset = NETMGR_REV_RMNET_START;
  for (i = 0; i < (unsigned char)num_rev_data_ports; i++)
  {
    ...
    {
      netmgr_log_med("%s(): rtrmnet created new iwlan vnd ", __func__, i);
    }
  }
...
)
{
  ...
  for (i = 0; i < num_rev_data_ports; i++)
  {
    ...
    {
      netmgr_log_err("%s(): rtrmnet del new iwlan vnd ", __func__, i);
    }
  }
