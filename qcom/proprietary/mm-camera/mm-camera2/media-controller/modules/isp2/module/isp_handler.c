  ...
  for (hw_index = 0; hw_index < ISP_HW_MAX; hw_index++) {
    ...
    if (isp_cfg_request->isp_id_mask & (1 << session_hw_id) &&
      ...
      if (isp_cfg_request->fetch_cfg_cmd == ISP_FETCH_PIPELINE_START) {
        ...
        isp_cfg_request->cds_enable = session_param->fetch_eng_cfg_data.cds_enable;
  ...
  mct_port_t *port, mct_event_t *event)
{
  ...
  /*Send CDS Update*/
  if (session_param->fetch_eng_cfg_data.cds_enable) {
    memset(&isp_cds_request, 0, sizeof(isp_cds_request));
    isp_cds_request.cds_request_info.need_cds_subsample = 1;
    isp_cds_request.cds_request_info.v_scale = 2;
    isp_cds_request.cds_request_info.w_scale = 2;
    isp_cds_request.cds_request = FALSE;
    copy_event.type = MCT_EVENT_MODULE_EVENT;
    copy_event.u.module_event.type = MCT_EVENT_MODULE_ISP_CDS_REQUEST;
    copy_event.u.module_event.module_event_data = (void *)&isp_cds_request;
    copy_event.identity = event->identity;
    copy_event.direction = MCT_EVENT_DOWNSTREAM;
    ret = isp_util_forward_event_to_internal_pipeline(session_param,
      &copy_event, session_param->offline_hw_id[0]);
  }
  ...
  mct_event_t *event)
{
  ...
  if (event->u.module_event.type == MCT_EVENT_MODULE_BUF_DIVERT_ACK) {
    return ret;
  }
