...
#include <stdio.h>
#include "ov5693_lib.h"
#include "debug_lib.h"

/**
 * FUNCTION: ov5693_real_to_register_gain
 *
 * DESCRIPTION: Calcuate sensor analog gain register value
 **/
static uint32_t ov5693_real_to_register_gain(float real_gain)
{
    uint32_t reg_gain = 0;
...
/**
 * FUNCTION: ov5693_register_to_real_gain
 *
 * DESCRIPTION: Calcuate sensor real gain value
 **/
static float ov5693_register_to_real_gain(uint32_t reg_gain)
{
    float gain;
...
/**
 * FUNCTION: ov5693_digital_gain_calc
 *
 * DESCRIPTION: Calcuate the sensor digital gain
 **/
static uint32_t ov5693_digital_gain_calc(float real_gain, float sensor_real_gain)
{
    uint32_t reg_dig_gain = ov5693_MIN_DGAIN_REG_VAL;
    float real_dig_gain = ov5693_MIN_DGAIN;

    if(real_gain > ov5693_MAX_AGAIN)
    {
        real_dig_gain = real_gain / sensor_real_gain;
    }
    else
    {
        real_dig_gain = ov5693_MIN_DGAIN;
    }

    if(real_dig_gain > ov5693_MAX_DGAIN)
    {
        real_dig_gain = ov5693_MAX_DGAIN;
    }
  ...
  __attribute__((unused)) float s_real_gain)
{
    ...
    exp_info->reg_gain = ov5693_real_to_register_gain(real_gain);
    exp_info->sensor_real_gain =
      ov5693_register_to_real_gain(exp_info->reg_gain);
    exp_info->sensor_digital_gain =
      ov5693_digital_gain_calc(real_gain, exp_info->sensor_real_gain);
    exp_info->sensor_real_dig_gain =
      (float)exp_info->sensor_digital_gain / ov5693_MAX_DGAIN_DECIMATOR;
...
/**
 * FUNCTION: ov5693_fill_exposure_array
 *
