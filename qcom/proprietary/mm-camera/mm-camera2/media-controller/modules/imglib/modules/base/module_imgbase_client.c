  ...
  bool internal_buf)
{
  ...
  for (i = 0; i < pframe->frame[0].plane_cnt; i++) {
    ...
    /* update buf divert */
    p_buf_divert->plane_vaddr[0] = (uint32_t)QIMG_ADDR(pframe, i);
  ...
  p_buf_divert->fd = fd;
  p_buf_divert->vaddr = &p_buf_divert->plane_vaddr[0];
  ...
  bool fetch_buf)
{
  ...
  double v_scale_factor[MAX_PLANE_CNT];
  ...
  isp_buf_divert_t *p_buf_divert)
{
  ...
  rc = module_imgbase_client_get_outputbuf(p_client, p_out_frame,
    p_mod->caps.use_internal_bufs, true);
  if (IMG_ERROR(rc)) {
  ...
  img_ack_event_t ack_event, void* data)
{
  ...
  case IMG_EVT_ACK_FORCE_RELEASE: {
    ...
    IDBG_LOW("[%s] IMG_EVT_ACK_FORCE_RELEASE, frameid %d id %x",
    ...
    buff_divert_ack.identity = p_ack->identity;
