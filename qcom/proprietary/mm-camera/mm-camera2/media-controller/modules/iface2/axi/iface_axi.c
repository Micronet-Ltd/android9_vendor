...
#ifdef VFE_EVENT_SOF2
	dev_idx = (uint32_t)sof->u.sof_info.dev_idx;
#else
    dev_idx = (uint32_t)axi_hw->dev_idx;
#endif
  IFACE_DBG("Event SOF session %x VFE%d, src %d with frame_id %d ksofid: %d",
       axi_hw->intf_param[input_src].session_id, dev_idx, input_src,
       sof->frame_id, ksofid);
  rc = axi_hw->notify_ops->notify(
      (void*)axi_hw->notify_ops->parent, dev_idx,
