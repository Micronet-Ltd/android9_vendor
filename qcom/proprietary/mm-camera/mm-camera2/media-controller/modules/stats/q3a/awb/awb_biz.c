...
static boolean awb_biz_estimate_ccm(void *obj, float cct, awb_ccm_type *ccm);
static void awb_biz_reset_flash_info(awb_biz_t *awb);
static boolean awb_biz_is_longshot_flash(awb_biz_t *awb);
static void awb_biz_handle_manual_and_lock(void *obj);
...
inline static chromatix_awb_all_light_type map_decision_from_q3a_to_biz(
  const awb_illuminant_type decision)
{
  chromatix_awb_all_light_type mapped_decision = AGW_AWB_INVALID_LIGHT;

  if (decision < AWB_ILLUMINANT_LAST)
  {
    mapped_decision =
      (chromatix_awb_all_light_type)decision_map_q3a_to_biz[decision];
  }
 ...
 *
 */
static void awb_biz_load_led_calibration_data(awb_led_calibaration_data_t* pCalib) {
  int i = 0;

  if (!pCalib) return;
  /* Reset the  calibration data */
  memset(&pCalib->single_led_data, 0, sizeof(pCalib->single_led_data));
  memset(pCalib->dual_led_data, 0, sizeof(pCalib->dual_led_data));
  ...
  if (file){
    ...
    for (i=0; i < AWB_DUAL_LED_CALIB_ON_STATE_COUNT; i++){
      ...
      if (fread(&pCalib->dual_led_data[i].rg_ratio, sizeof(float), 1, file) &&
          fread(&pCalib->dual_led_data[i].bg_ratio, sizeof(float), 1, file)) {
        pCalib->dual_led_data[i].is_valid = true;
        AWB_LOW("Loaded dual LED calibration data[%d]: %f, %f",
          i, pCalib->dual_led_data[i].rg_ratio, pCalib->dual_led_data[i].bg_ratio);
      } else {
  ...
  /* Try to load single led calibration data from file */
  file = fopen(SINGLE_LED_CALIB_FILE_PATH, "rb");
  if (file){
    AWB_HIGH("Found single LED calibration data file. Loading...");
    /* Read the r/g and b/g ratios from file */
    if (fread(&pCalib->single_led_data.rg_ratio, sizeof(float), 1, file) &&
        fread(&pCalib->single_led_data.bg_ratio, sizeof(float), 1, file)) {
      pCalib->single_led_data.is_valid = true;
      AWB_LOW("Loaded single LED calibration data: %f, %f",
        pCalib->single_led_data.rg_ratio, pCalib->single_led_data.bg_ratio);
    } else {
      AWB_ERR("Error reading from the single LED calibration file!");
    }
    fclose(file);
  } else {
    AWB_HIGH("Could not find the single LED calibration data file.");
  }
}
 ...
 *
 */
static void awb_biz_save_led_calibration_data(awb_biz_t *awb,
  awb_led_calibaration_data_t* pCalib)
{
  int i = 0;

  /* Only save calibration data if calibration mode is enabled
   * and the data is not already saved to file */
  if (!pCalib || awb->led_calib_done == TRUE) {
    return;
  }

  if (pCalib->mode == Q3A_LED_CALIB_MODE_DUAL) {
    /* Only save the calibration data if it's valid */
    for (i = 0; i < AWB_DUAL_LED_CALIB_ON_STATE_COUNT; i++) {
      if (!pCalib->dual_led_data[i].is_valid){
        return;
      }
    }
    FILE* file = fopen(DUAL_LED_CALIB_FILE_PATH, "w");
    if (file) {
      AWB_HIGH("Saving dual LED calibration data file...");
      for (i = 0; i < AWB_DUAL_LED_CALIB_ON_STATE_COUNT; i++) {
        /* Write the r/g and b/g ratios to file */
        if (!fwrite(&pCalib->dual_led_data[i].rg_ratio, sizeof(float), 1, file) ||
            !fwrite(&pCalib->dual_led_data[i].bg_ratio, sizeof(float), 1, file)) {
          AWB_ERR("Error! Failed to write to dual LED calibration file!");
          break;
        }
        AWB_HIGH(" Succeed write to dual LED calibration file!");
        awb->led_calib_done = TRUE;
      }
      fclose(file);
    } else {
      AWB_ERR("Could not open to the dual LED calibration data file to save the data.");
    }
  } else if (pCalib->mode == Q3A_LED_CALIB_MODE_SINGLE) {
    /* Only save the calibration data if it's valid */
    if (!pCalib->single_led_data.is_valid) {
      return;
    }

    FILE* file = fopen(SINGLE_LED_CALIB_FILE_PATH, "w");
    if (file){
      AWB_HIGH("Saving single LED calibration data file...");
      /* Write the r/g and b/g ratios to file */
      if (!fwrite(&pCalib->single_led_data.rg_ratio, sizeof(float), 1, file) ||
          !fwrite(&pCalib->single_led_data.bg_ratio, sizeof(float), 1, file)) {
        AWB_ERR("Error! Failed to write to single LED calibration file!");
      } else {
        AWB_HIGH(" Succeed write to single LED calibration file!");
        awb->led_calib_done = TRUE;
      }
      fclose(file);
    } else {
      AWB_ERR("Could not open to the single LED calibration data file to save the data.");
    }
  }
}
 ...
 * @return boolean
 */
static boolean awb_biz_set_init(awb_biz_t *awb,awb_set_parameter_init_t* init_param)
{
  ...
  awb_led_calibaration_data_t led_calibration_data;
  memset(&led_calibration_data, 0, sizeof(led_calibration_data));

  if (!awb || !init_param)
    return FALSE;

  chromatix = (chromatix_3a_parms_type*)init_param->chromatix;
  if (!chromatix)
    return FALSE;
  ...
  if (awb->led_calib_mode == Q3A_LED_CALIB_MODE_OFF) {
    awb_biz_load_led_calibration_data(&led_calibration_data);
  }
  led_calibration_data.mode = awb->led_calib_mode;
  led_calibration_data.dual_led_state = AWB_DUAL_LED_CALIB_OFF;
  ...
  set_param.u.tuning_params.led_calib_data = &led_calibration_data;
  set_param.u.tuning_params.tuning_params = &chromatix->AWB_bayer_algo_data;
  set_param.u.tuning_params.reserved_tuning_param = &reserved_params;
  set_param.u.tuning_params.initial_white_balance = &chromatix->AWB_bayer_algo_data.awb_MWB.MWB_d50;
  set_param.u.tuning_params.led_flash_white_balance = &chromatix->AWB_bayer_algo_data.awb_MWB.led_flash_white_balance;
  set_param.u.tuning_params.stored_params = init_param->stored_params;

  awb->awb_full_camif_enable = chromatix->AWB_bayer_algo_data.awb_reserved_data.reserved_int[18];
  ...
  awb->chromatix_tl84_white_balance  = chromatix->AWB_bayer_algo_data.awb_MWB.MWB_tl84;
  awb->chromatix_d50_white_balance = chromatix->AWB_bayer_algo_data.awb_MWB.MWB_d50;
  awb->chromatix_d65_white_balance = chromatix->AWB_bayer_algo_data.awb_MWB.MWB_d65;
  awb->chromatix_incandescent_white_balance = chromatix->AWB_bayer_algo_data.awb_MWB.MWB_A;
  awb->gain_adj = chromatix->AWB_bayer_algo_data.awb_basic_tuning.awb_adjust_gain_table.gain_adj_normallight;
  awb->snow_blue_gain_adj_ratio = chromatix->AWB_bayer_algo_data.awb_blue_tuning.snow_blue_gain_adj_ratio;
  awb->beach_blue_gain_adj_ratio = chromatix->AWB_bayer_algo_data.awb_blue_tuning.beach_blue_gain_adj_ratio;
  awb->awb_ccm_enable = chromatix->AWB_bayer_algo_data.awb_CCM_control.awb_ccm_enable;
#if defined(CHROMATIX_VERSION) && (CHROMATIX_VERSION == 0x0309)
  awb->dual_LED_enable = chromatix->AWB_bayer_algo_data.awb_led_tuning.mix_LED_table.dual_led_enable;
#else
  //TODO:310 Revisit after dual led is moved to AEC
  awb->dual_LED_enable = 0;//chromatix->AWB_bayer_algo_data.awb_led_tuning.mix_LED_table.dual_led_enable;
#endif
  if (!awb->first_init)
    return TRUE;
  ...
  if (awb->awb_ccm_enable) {
    ...
    const q3a_core_result_type result =
      awb_biz_estimate_ccm((void*)awb, awb->color_temp,
        &(awb->awb_flash_off_settings.ccm));
...
static boolean awb_biz_estimate_ccm(void *obj, float cct,
  ...
  if (rc == Q3A_CORE_RESULT_SUCCESS) {
    ...
    memcpy(ccm, &(get_param_output.u.ccm), sizeof(awb_ccm_type));
  }
  return rc;
}
  ...
  if (TRUE == awb->dual_LED_enable) {
    ...
    if (rc == Q3A_CORE_RESULT_SUCCESS) {
      awb->dual_led_settings.led1_high_setting =
        get_param_output.u.led_data.dual_led_settings.led1_high_setting;
      awb->dual_led_settings.led1_low_setting =
        get_param_output.u.led_data.dual_led_settings.led1_low_setting;
      awb->dual_led_settings.led2_high_setting =
        get_param_output.u.led_data.dual_led_settings.led2_high_setting;
      awb->dual_led_settings.led2_low_setting =
        get_param_output.u.led_data.dual_led_settings.led2_low_setting;
      awb->dual_led_flux_gain = get_param_output.u.led_data.dual_led_flux_gain;
...
static int awb_biz_set_current_wb(awb_biz_t *awb, uint32_t new_wb)
{
  ...
  case CAMERA_WB_CLOUDY_DAYLIGHT:
    ...
    gain_adjust = gain_adj[AGW_AWB_D65];
    awb->decision = AGW_AWB_D65;
    awb->color_temp = 6500;
    break;
  ...
  case CAMERA_WB_INCANDESCENT: /* a.k.a "A",  Turn off auto WB */
    ...
    gain_adjust = gain_adj[AGW_AWB_A];
    awb->decision = AGW_AWB_A;
    awb->color_temp = 2800;
    break;
  ...
  case CAMERA_WB_WARM_FLUORESCENT:
    ...
    gain_adjust = gain_adj[AGW_AWB_WARM_FLO];
    awb->decision = AGW_AWB_WARM_FLO;
    awb->color_temp = 4100;
    break;
  ...
  case CAMERA_WB_DAYLIGHT: /* a.k.a "D65", Turn off auto WB */
    ...
    gain_adjust = gain_adj[AGW_AWB_D50];
    awb->decision = AGW_AWB_D50;
    awb->color_temp = 5000;
    break;
  ...
  case CAMERA_WB_SHADE:
    awb->awb_update = TRUE;
    rgb_gains = awb->chromatix_d65_white_balance;
    gain_adjust = gain_adj[AGW_AWB_D75];
    awb->decision = AGW_AWB_D75;
    awb->color_temp = 7500;
    break;
  ...
  awb_output_data_t *output)
{
  ...
  if (!awb_biz_translate_dim_fov2camif(awb, &width, &height)) {
    AWB_ERR("invalid inputs to translate dimensions for  fov 2 camif");
  ...
  if (bg_config->roi.left != left ||
    ...
    memcpy(&output->config, config, sizeof(awb_config_t));
  } else {
...
static void  awb_biz_pack_output(awb_biz_t *awb, awb_output_data_t *output)
{
  ...
  memcpy(output->samp_decision, awb->sample_decision, sizeof(int) * AWB_DECISION_MAP_SIZE);
 ...
 /* Get eztune information from core if eztune enabled */
  ...
  if (awb->ez_running) {
    ...
    memcpy(output->eztune_data.samp_decision, awb->sample_decision, sizeof(int) * 64);
  }
...
/**
 * awb_biz_set_led_calibration_mode
 *
 ...
 *
 */
static boolean awb_biz_set_led_calibration_mode(
  awb_biz_t* awb, q3a_led_calib_mode_t mode)
{
  q3a_core_result_type rc = Q3A_CORE_RESULT_SUCCESS;
  awb_set_param_info_type set_param_info;
  if (awb->led_calib_mode != mode) {
    AWB_HIGH("Setting LED calibration mode to: %d", mode);
    /* Set dual LED calibration mode inside core. */
    set_param_info.param_type = AWB_SET_PARAM_LED_CALIBRATION_MODE;
    set_param_info.u.led_calib_mode = mode;
    ...
    awb->led_calib_mode = mode;
    awb->led_calib_done = FALSE;
...
*
**/
static boolean awb_biz_set_params(awb_set_parameter_t *param, awb_output_data_t *output, void *awb_obj)
{
  boolean rc            = TRUE;
  ...
  AWB_LOW("paramt type =%d", param->type);
  if (!param || !awb_obj) {
    rc = FALSE;
    goto done;
  }
  ...
  case AWB_SET_PARAM_INIT_CHROMATIX_SENSOR:
    rc = awb_biz_set_init(awb, &param->u.init_param);
    break;
  ...
  case AWB_SET_PARM_FAST_AEC_DATA: {
    ...
    break;
  case AWB_SET_PARAM_LED_CALIB_MODE: {
    rc = awb_biz_set_led_calibration_mode(awb, param->u.led_calib_mode);
  }
    break;
  ...
  void *awb_obj)
{
  ...
  case AWB_UNIFIED_FLASH:
  {
    if (AWB_AUTO == awb->current_wb_type) {
      ...
      chromatix_manual_white_balance_type led_off_gains;
      chromatix_manual_white_balance_type led_on_gains;
      chromatix_manual_white_balance_type output_gains;
...
static void awb_biz_pack_process_output(awb_biz_t *awb)
{
  ...
  AWB_LOW("(r,g,b) gain = (%f, %f, %f), cct = %d, decision = %d",
    ...
    awb->decision);
  awb->core_output.ccm = core_output->ccm;
  awb->snap_gains = core_output->snap_gains;
  awb->snap_color_temp = core_output->snap_color_temp;
#if defined(CHROMATIX_VERSION) && (CHROMATIX_VERSION == 0x0309)
  awb->dual_led_settings = core_output->led_data.dual_led_settings;
  awb->dual_led_flux_gain = core_output->led_data.dual_led_flux_gain;
#endif

  /* Save the Dual LED Calibration data if calibration is enabled */
  if (core_output->led_data.led_calib_data.mode != Q3A_LED_CALIB_MODE_OFF)
    awb_biz_save_led_calibration_data(awb, &core_output->led_data.led_calib_data);
...
*
**/
static void awb_biz_process(stats_t *stats, void *obj, awb_output_data_t *output)
{
  q3a_core_result_type rc = Q3A_CORE_RESULT_SUCCESS;
  awb_biz_t *awb = (awb_biz_t *)obj;
  ...
  if (Q3A_STATS_STREAM_OFFLINE == stats->isp_stream_type) {
    ...
    if (awb->current_wb_type == AWB_AUTO && !awb->awb_locked) {
      ...
      awb_biz_handle_manual_and_lock(obj);
    }
    return;
  } else {
    AWB_LOW("op_mode =%d", awb->op_mode);
    if (0 == awb->op_mode)
    {
    ...
    AWB_LOW("E awb->op_mode=%d, type=%d, lock=%d", awb->op_mode,
      awb->current_wb_type, awb->awb_locked);
