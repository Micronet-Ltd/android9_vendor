...
// LPM transitions should not be handled by AF
// in case of HAL1
  ...
  if (!((af_port->af_master_focus_info.focusStatus == AF_CORE_STATUS_FOCUSED &&
     info->focusStatus == AF_CORE_STATUS_NOT_FOCUSED) ||
     (af_port->af_master_focus_info.focusStatus == AF_CORE_STATUS_NOT_FOCUSED &&
     info->focusStatus == AF_CORE_STATUS_FOCUSED))) {
     isValidTransition = TRUE;
  }

  if((af_port->focusedAtLeastOnce == FALSE) ||
    ((af_port->af_master_focus_info.focusStatus != info->focusStatus) && isValidTransition)) {
  ...
  af_output_data_t *af_out)
{
  ...
  if((af_out->swaf_config.enable == FALSE) &&
    ...
    (af_port->hal_version != CAM_HAL_V1)) {
  ...
  void *output)
{
  ...
  do {
    ...
    if (NULL == depth_output) {
      AF_HIGH("output is null, skip stats");
      break;
    }

    af_port = (af_port_private_t *)port->port_private;
    if (NULL == af_port) {
      AF_ERR("Null private port pointer");
      break;
    }

    if (af_port->thread_data->no_stats_mode) {
  ...
  /* Ack the isp buffer back and free memory */
  af_port_send_isp_buffer_divert_ack(port, stats_buff);
  free(stats_buff);
  ...
  af_port_private_t * af_port, isp_buf_divert_t * stats_buff)
{
  ...
  memset(af_msg, 0 , sizeof(q3a_thread_af_msg_t));
  af_msg->type = MSG_AF_SET;
  af_msg->u.af_set_parm.type = AF_SET_PARAM_PDAF_DATA;
 ...
 case MCT_EVENT_MODULE_IMGLIB_DCRF_OUTPUT: {
  ...
  case MCT_EVENT_MODULE_ISP_DIVERT_TO_3A: {
    ...
    isp_buf_divert_t *isp_buff =
      (isp_buf_divert_t *)malloc(sizeof(isp_buf_divert_t));
    if (isp_buff == NULL) {
      AF_ERR("Error allocating memory.");
      break;
    }
    ...
    if (rc == TRUE) {
      /* rc is TRUE means isp buffer will be handled in af thread,
       * thus set ack_flag to FALSE to wait af thread handle and ack back */
      stats_buff->ack_flag = FALSE;
    } else {
      /* free the isp_buff if enqueue failed */
      free(isp_buff);
    }
  }
    break;
...
static void af_port_handle_control_event(mct_port_t * port,
  ...
  case MCT_EVENT_CONTROL_UNLINK_INTRA_SESSION: {
    ...
    boolean rc = FALSE;
    mct_port_t *peer_port = NULL;
    rc = stats_util_get_peer_port(event, af_port->intra_peer_id, port,
      &peer_port);
    if (rc == FALSE) {
      AF_HIGH("Peer Port already unlinked");
    } else {
      af_port_private_t *af_peer_port = (af_port_private_t *)peer_port->port_private;
      MCT_PORT_INTRAUNLINKFUNC(peer_port);
      af_peer_port->intra_peer_id = 0;
    }
