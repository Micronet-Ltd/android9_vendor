...
LOCAL_CFLAGS  += -Werror -Wunused-parameter -Wno-implicit-function-declaration
...
CAM_SRC_FILES := $(shell find $(LOCAL_SRC_DIR) -type d \( -name gyro -o -name af -o -name af_v2 \) -prune -o -name '*.c' | sed s:^$(LOCAL_PATH)::g)
CAM_SRC_FILES += $(shell find $(LOCAL_SRC_DIR) -type d \( -name gyro -o -name af -o -name af_v2 \) -prune -o -name '*.cpp' | sed s:^$(LOCAL_PATH)::g)
...
#Shared libraries
LOCAL_SHARED_LIBRARIES := libdl libcutils libmmcamera2_mct libmmcamera2_is libmmcamera2_stats_algorithm
