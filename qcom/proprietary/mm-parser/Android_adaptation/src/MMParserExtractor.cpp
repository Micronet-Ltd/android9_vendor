...
bool FileSourceWrapper::IsDivxSupportEnabled(
    ...
    /* By Default this flag will be set to true.
       ...
       "vendor.mm.enable.qcom_parser" property is extended to enable/disable these
       features. */
    ...
    property_get("vendor.mm.enable.qcom_parser", ucDivxPropertyValue, "0");
    ...
    switch (minorType) {
       ...
       case FILE_SOURCE_MN_TYPE_VP9:
        ...
        case FILE_SOURCE_MN_TYPE_HEIF:
            ...
            return MEDIA_MIMETYPE_IMAGE_ANDROID_HEIC;
        default:
...
const char *fillMimeType(FileSourceFileFormat fileFormat, uint8 tracksPresent)
{
    switch(fileFormat) {
       ...
       case FILE_SOURCE_DSDIFF:
        ...
        case FILE_SOURCE_MPEG4:
            if(tracksPresent & VIDEO_TRACK) {
                ...
                return MEDIA_MIMETYPE_CONTAINER_HEIF;
            }
...
MMParserExtractor::MMParserExtractor(DataSourceBase *source, void* meta, FileSourceFileFormat format)
    : mDataSource(source),
...
status_t MMParserExtractor::getTrackMetaData(MetaDataBase& meta,
...
status_t MMParserExtractor::getMetaData(MetaDataBase& meta) {
...
status_t MMParserExtractor::readMetaData() {
    ...
    uint32_t audioParserInputSize = 0;
    ...
    if (gbLocalPlayback)
    {
        ...
        audioParserInputSize = atoi(audioParserBuf);
        if (audioParserInputSize > MAX_PARSER_IN_BUF_SIZE)
            audioParserInputSize = 0;
    }
            ...
            if (majorType == FILE_SOURCE_MJ_TYPE_AUDIO){
                ...
                if( pMimeType == MEDIA_MIMETYPE_AUDIO_AAC ) {
                    ...
                    if( minorType == FILE_SOURCE_MN_TYPE_AAC_ADIF ) {
                         ...
                         //set parameters assuming that it may work in byte mode
                         if (audioParserInputSize && fileFormat == FILE_SOURCE_AAC) {
                             track->meta.setInt32(kKeyIsADTS, 1);
                             track->meta.setInt32(kKeyMaxInputSize, audioParserInputSize);
                             configStatus = m_pFileSourceWrapper->SetConfiguration(trackInfo.id, &pitem, FILE_SOURCE_MEDIA_RETAIN_AUDIO_HEADER);
                         }
                ...
                if( (minorType == FILE_SOURCE_MN_TYPE_AC3) ||
                    ...
                    //set parameters assuming that it may work in byte mode
                    if (audioParserInputSize && fileFormat == FILE_SOURCE_MP3) {
                        track->meta.setInt32(kKeyMaxInputSize, audioParserInputSize);
                    }
                    configStatus = m_pFileSourceWrapper->SetConfiguration(trackInfo.id, &pitem, FILE_SOURCE_MEDIA_OUTPUT_SINGLE_AUDIO_FRAME);
...
//! If any customer needs downgrade, then on need basis decision to be taken
                ...
                if (mediaTrackInfo.videoTrackInfo.ulFrameCount)
                  track->meta.setInt32(kKeyFrameCount, mediaTrackInfo.videoTrackInfo.ulFrameCount);
                    ...
                    if(buffer!=NULL) {
                        ...
                        if(status==FILE_SOURCE_SUCCESS) {
                            if (false == bRawCodecData) {
                                ...
                                track->meta.setData(kKeyVp9CodecPrivate, 0, buffer, nFormatBlockSize);
                            }
                        } else {
                            LOGE(" getFormatBlock failed \n");
                        }
            ...
            else if (majorType == FILE_SOURCE_MJ_TYPE_IMAGE &&
                     minorType == FILE_SOURCE_MN_TYPE_HEIF)
            {
                trackPresent |= IMAGE_TRACK;
                setHEIFTrackMeta(track, mediaTrackInfo);
            }
        }
    }//for loop

    const char* pFileType = fillMimeType(fileFormat, trackPresent);
    if(NULL != pFileType) {
        mFileMetaData.setCString(kKeyMIMEType, pFileType);
        LOGV("fillMimeType:: %s",(char*)pFileType);
        Track* currentTrack = mFirstTrack;
        while (currentTrack != NULL) {
            currentTrack->meta.setCString(kKeyFileFormat, pFileType);
...
void MMParserExtractor::setCodecSpecificData(Track* track, uint32 key, uint32 dataType,
    ...
    if (e == FILE_SOURCE_SUCCESS && dataSize) {
        ...
        if (pBuffer) {
            ...
            if (e == FILE_SOURCE_SUCCESS)
                track->meta.setData(key, dataType, pBuffer, dataSize);
            delete[] pBuffer;
        }
    }
}
void MMParserExtractor::setHEIFTrackMeta(Track* track, MediaTrackInfo& info) {
    track->meta.setInt32(kKeyTrackID, track->trackId);
    track->meta.setInt32(kKeyWidth, info.sImageTrackInfo.ulImageWidth);
    track->meta.setInt32(kKeyHeight, info.sImageTrackInfo.ulImageHeight);
    track->meta.setInt32(kKeyMaxInputSize, info.sImageTrackInfo.ulMaxBufSize);
    if (info.sImageTrackInfo.ucRotation)
        track->meta.setInt32(kKeyRotation, info.sImageTrackInfo.ucRotation);
    if (info.sImageTrackInfo.bIsPrimary)
    {
        track->meta.setInt32(kKeyTrackIsDefault, 1);
        if (info.sImageTrackInfo.ulExifSize)
        {
            mFileMetaData.setInt64(kKeyExifOffset, (int64_t)info.sImageTrackInfo.ullExifOffset);
            mFileMetaData.setInt64(kKeyExifSize, (int64_t)info.sImageTrackInfo.ulExifSize);
        }
    }
    if (info.sImageTrackInfo.ulThumbnailWidth && info.sImageTrackInfo.ulThumbnailHeight)
    {
        track->meta.setInt32(kKeyThumbnailWidth, info.sImageTrackInfo.ulThumbnailWidth);
        track->meta.setInt32(kKeyThumbnailHeight, info.sImageTrackInfo.ulThumbnailHeight);
        setCodecSpecificData(track, kKeyThumbnailHVCC, kTypeHVCC,
                             FS_FORMAT_DATA_HEIF_THUMBNAIL_HVCC);
    }
    if (info.sImageTrackInfo.bIsGrid)
    {
        track->meta.setInt32(kKeyGridRows, info.sImageTrackInfo.ucGridRows);
        track->meta.setInt32(kKeyGridCols, info.sImageTrackInfo.ucGridColumns);
        track->meta.setInt32(kKeyTileWidth, info.sImageTrackInfo.ulTileWidth);
        track->meta.setInt32(kKeyTileHeight, info.sImageTrackInfo.ulTileHeight);
    }
    setCodecSpecificData(track, kKeyHVCC, kTypeHVCC, FS_FORMAT_DATA_HEIF_HVCC);
    setCodecSpecificData(track, kKeyIccProfile, 0, FS_FORMAT_DATA_HEIF_ICC);
}
...
MediaTrack* MMParserExtractor::getTrack(size_t index) {
...
status_t MMParserSource::start(MetaDataBase *params) {
    CHECK(!mStarted);
    ...
    LOGV("Max frame size = %d", max_size);
    if (mMinorType == FILE_SOURCE_MN_TYPE_AAC_ADIF) {
        //Parser does know frame boundaries for ADIF
        // using twice of default size 1536 to accomodate incomplete frames
        max_size = 1536*2;
    } else if (FILE_SOURCE_MN_TYPE_G711_ALAW == mMinorType ||
               FILE_SOURCE_MN_TYPE_G711_MULAW == mMinorType ||
               FILE_SOURCE_MN_TYPE_HEIF == mMinorType) {
        int32_t ulSize = 0;
        if (mFormat.findInt32(kKeyMaxInputSize, &ulSize) && ulSize) {
            LOGV("Max frame size updated based on kKeyMaxInputSize %lu", ulSize);
            max_size = ulSize;
        }
    } else if (gbLocalPlayback && (mFileFormat == FILE_SOURCE_MP3 ||
                                   mFileFormat == FILE_SOURCE_AAC)) {
        //Check if byte mode is enabled and apply only in case of audio local playback usecases
        int32_t extraFlags = 0;
        bool useBigBuffer = false;
        FileSourceConfigItem pitem;
        if (params && params->findInt32(kKeyExtraFlag, &extraFlags) &&
            (extraFlags & USE_AUDIO_BIG_BUFFERS)) {
            // if big buffer mode enabled, override default buffer size
            char bufferSize[PROPERTY_VALUE_MAX];
            property_get("vendor.audio.parser.ip.buffer.size", bufferSize, "0");
            uint32_t audioInputBufferSize = atoi(bufferSize);
            if (audioInputBufferSize > 0 && audioInputBufferSize <= MAX_PARSER_IN_BUF_SIZE &&
                m_pFileSourceWrapper->SetConfiguration(mTrackId, &pitem,
                    FILE_SOURCE_MEDIA_OUTPUT_BYTES_STREAM) == FILE_SOURCE_SUCCESS) {
                useBigBuffer = true;
                max_size = audioInputBufferSize;
                params->setInt32(kKeyIsByteMode, 1);
                LOGD("Overriding parser out buffer size = %d", max_size);
            }
        }
        if (!useBigBuffer && m_pFileSourceWrapper->SetConfiguration(mTrackId, &pitem,
                       FILE_SOURCE_MEDIA_OUTPUT_SINGLE_AUDIO_FRAME) != FILE_SOURCE_SUCCESS) {
            LOGI("Unable to set single frame output mode config");
        }
    }
    ...
    if (max_size > MAX_MEDIA_BUFFER_SIZE)
      return ERROR_MALFORMED;
    ...
    if (!strncasecmp(mime, "video/", 6)) {
        ...
        mGroup->add_buffer(MediaBufferBase::Create(max_size + 10 *2 ));
        LOGV("MMParserSource::start decodedWidth = %d, decodedHeight = %d",decodedWidth,decodedHeight);
        LOGV("MMParserSource::start allocate buffer for video track size = %d", max_size);
    } else {
        mGroup->add_buffer(MediaBufferBase::Create(max_size));
        LOGV("MMParserSource::start allocate buffer for audio track size = %d", max_size);
    }
...
status_t MMParserSource::getFormat(MetaDataBase& format) {
