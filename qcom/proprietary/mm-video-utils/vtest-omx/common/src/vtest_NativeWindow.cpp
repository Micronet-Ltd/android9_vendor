...
NativeWindow::NativeWindow(OMX_BOOL bRotate,
...
#if (ANDROID_SDK_VERSION < ANDROID_P)
    ...
    if (m_bShowVideo) {
        //show-up video on the top
        transaction.setLayer(m_pControl, 100000 + 1);
    } else {
        //Don't show
        transaction.setLayer(m_pControl, -1);
    }
    transaction.apply();

    //show-up background below video
    transaction.setLayer(m_pBackgroundControl, 100000);
    transaction.show(m_pBackgroundControl);
    transaction.apply();
#endif
...
#else
    SurfaceComposerClient::Transaction transaction;
    transaction.setPosition(m_pControl, x, y);
    transaction.setSize(m_pControl, w, h);
    transaction.apply();
#endif
