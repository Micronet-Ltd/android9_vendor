    ...
    void *cookie, int32_t cmd_status)
{
    ...
    if (!p_gcs_ses->exit_buffering) {
        ...
        if (p_ses->enable_second_stage)
            pthread_rwlock_wrlock(&p_ses->pcm_rw_lock);
        ...
        if ((v_info->kw_capture_format & MULAW) &&
            ...
            if (status) {
                ...
                if (p_ses->enable_second_stage)
                    pthread_rwlock_unlock(&p_ses->pcm_rw_lock);
                goto exit_unlock;
            }
        ...
        if (buff_sz > free_bytes) {
            ALOGW("%s: buffer overflow: buff_size %d, free_bytes %d",
...
static void process_lab_capture(st_hw_session_t *p_ses)
{
    ...
    if (p_ses->enable_second_stage) {
        ...
        list_for_each_safe(node, tmp_node, p_ses->second_stage_list) {
             ...
             */
            st_sec_stage->ss_session->pcm_rw_lock = &(p_ses->pcm_rw_lock);
            pthread_rwlock_rdlock(st_sec_stage->ss_session->pcm_rw_lock);
            /*
            ...
            st_sec_stage->ss_session->hw_end_ptr = (char *)p_hw_ses->circ_buff_tail;
            st_sec_stage->ss_session->hw_start_ptr = (char *)p_hw_ses->circ_buff;
        ...
        if (!p_ses->client_req_preroll ||
            (!p_ses->is_generic_event && !p_ses->client_req_hist_buf))
    ...
    unsigned int bytes)
{
    ...
    st_hw_session_gcs_t *p_gcs_ses = (st_hw_session_gcs_t *)p_ses;
    struct timespec tspec;
    uint32_t copy_bytes = 0, move_bytes = 0, bytes_to_tail = 0, delay_bytes = 0;
    ...
    if (p_gcs_ses->move_client_ptr) {
        ...
        p_gcs_ses->rd_ptr = circ_buff_move_ptr_fwd(p_gcs_ses->circ_buff,
            p_gcs_ses->rd_ptr, move_bytes, p_gcs_ses->circ_buff_sz);
    ...
    while (bytes > 0 && !p_gcs_ses->exit_buffering) {
        ...
        if (0 == p_gcs_ses->unread_bytes) {
            ...
            GET_WAIT_TIMESPEC(tspec, (convert_bytes_to_ms((bytes * 2),
                &p_ses->config) * NSECS_PER_MSEC));
    ...
    sound_trigger_device_t *stdev)
{
    ...
    pthread_cond_init(&(p_hw_ses->circ_buff_cond), &cond_attr2);
