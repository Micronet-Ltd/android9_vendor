...
/* circular buffer utility function */
...
static int read_pcm_data(st_hw_session_lsm_t *p_ses,
                     ...
                     unsigned int bytes)
{
    unsigned int copy_bytes = 0, read_bytes = 0, move_bytes = 0, delay_bytes = 0;
    ...
    if (p_ses->move_client_ptr) {
        ...
        move_bytes = MIN(delay_bytes, p_ses->pcm_out_buf_unread_bytes);
        ALOGD("%s: Moving client ptr by %d bytes", __func__, move_bytes);
        p_ses->pcm_out_buf_rd_ptr = circ_buff_move_ptr_fwd(p_ses->pcm_out_buf_start_ptr,
            p_ses->pcm_out_buf_rd_ptr, move_bytes, p_ses->pcm_out_buf_size);
        p_ses->pcm_out_buf_unread_bytes -= move_bytes;
    ...
    while (!p_ses->exit_lab_processing && (bytes > 0)) {
        if (!p_ses->pcm_out_buf_unread_bytes) {
            ...
            GET_WAIT_TIMESPEC(tspec,
                (convert_bytes_to_ms((bytes * 2), &p_ses->common.config) *
                NSECS_PER_MSEC));
                     ...
                     unsigned int bytes)
{
    ...
    unsigned char *updated_wr_ptr = NULL;
    st_arm_second_stage_t *st_sec_stage = NULL;

    if (p_lsm_ses->common.enable_second_stage)
        pthread_rwlock_wrlock(&p_lsm_ses->common.pcm_rw_lock);
    ...
    while (!p_lsm_ses->exit_lab_processing && (bytes > 0)) {
        ...
        if (!free_bytes) {
            ...
            if (p_lsm_ses->exit_lab_processing) {
                ...
                pthread_mutex_unlock(&p_lsm_ses->pcm_out_buf_lock);
                if (p_lsm_ses->common.enable_second_stage)
                    pthread_rwlock_unlock(&p_lsm_ses->common.pcm_rw_lock);
                return -EIO;
            }
        }
        ...
        list_for_each_safe(node, tmp_node, p_lsm_ses->common.second_stage_list) {
            ...
            if ((p_lsm_ses->bytes_written >= st_sec_stage->ss_session->buf_start) &&
                ...
                st_sec_stage->ss_session->hw_rd_ptr =
                    (char *)p_lsm_ses->pcm_out_buf_wr_ptr - write_bytes;
                st_sec_stage->ss_session->start_processing = true;
            }

            if (st_sec_stage->ss_session->start_processing) {
                st_sec_stage->ss_session->unread_bytes += write_bytes;
    ...
    if (p_lsm_ses->common.enable_second_stage) {
        ...
        list_for_each_safe(node, tmp_node, p_lsm_ses->common.second_stage_list) {
            ...
            if ((p_lsm_ses->bytes_written >= st_sec_stage->ss_session->buf_start) &&
                !st_sec_stage->ss_session->start_processing) {
                st_sec_stage->ss_session->hw_rd_ptr =
                    (char *)p_lsm_ses->pcm_out_buf_wr_ptr - bytes;
                st_sec_stage->ss_session->start_processing = true;
            }
    ...
    uint32_t cnn_append_bytes, uint32_t vop_append_bytes)
{
    ...
    list_for_each_safe(node, tmp_node, p_ses->second_stage_list) {
        ...
        pthread_rwlock_wrlock(st_sec_stage->ss_session->pcm_rw_lock);
...
void process_raw_lab_data_cpe(st_hw_session_lsm_t *p_lsm_ses)
{
    ...
    if (p_lsm_ses->common.enable_second_stage) {
        ...
        list_for_each_safe(node, tmp_node, p_lsm_ses->common.second_stage_list) {
            ...
            st_sec_stage->ss_session->pcm_rw_lock = &(p_lsm_ses->common.pcm_rw_lock);
            pthread_rwlock_rdlock(st_sec_stage->ss_session->pcm_rw_lock);
            /*
            ...
            st_sec_stage->ss_session->hw_end_ptr = (char *)p_lsm_ses->pcm_out_buf_end_ptr;
            st_sec_stage->ss_session->hw_start_ptr = (char *)p_lsm_ses->pcm_out_buf_start_ptr;
        ...
        if (!p_lsm_ses->common.client_req_preroll ||
            (!p_lsm_ses->common.is_generic_event && !p_lsm_ses->common.client_req_hist_buf))
...
static int route_stop_buffering_ape(st_hw_session_t* p_ses, bool capture_requested __unused)
{
    ...
    while (p_lsm_ses->lab_processing_active) {
        ...
        if (status) {
            ...
            /* retry once more after aborting ongoing read with pcm_stop/start */
    ...
    sound_trigger_device_t *stdev)
{
    ...
    pthread_cond_init(&p_lsm_ses->pcm_out_buf_cond, &c_attr_buf);

    if (p_ses->enable_second_stage)
        pthread_rwlock_init(&(p_ses->pcm_rw_lock), NULL);
...
void st_hw_sess_lsm_deinit(st_hw_session_t *const p_ses)
{
    ...
    pthread_cond_destroy(&p_lsm_ses->pcm_out_buf_cond);
    pthread_mutex_destroy(&p_lsm_ses->pcm_out_buf_lock);

    if (p_ses->enable_second_stage)
        pthread_rwlock_destroy(&p_ses->pcm_rw_lock);
}
