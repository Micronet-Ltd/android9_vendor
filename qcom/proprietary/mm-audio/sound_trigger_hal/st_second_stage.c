...
static int process_frame_keyword_detection(st_arm_ss_session_t *ss_session,
    char *frame, capi_v2_stream_data_t *stream_input,
...
static int start_keyword_detection(st_arm_second_stage_t *st_sec_stage)
{
    ...
    pthread_rwlock_rdlock(ss_session->pcm_rw_lock);
    while (!ss_session->exit_buffering &&
        ...
        if (ss_session->unread_bytes < ss_session->buff_sz) {
            ...
            pthread_rwlock_unlock(ss_session->pcm_rw_lock);
            ret = pthread_cond_timedwait(&ss_session->cond, &ss_session->lock, &tspec);
            pthread_rwlock_rdlock(ss_session->pcm_rw_lock);
        ...
        if (ss_session->hw_end_ptr < (ss_session->hw_rd_ptr + ss_session->buff_sz)) {
            copy_bytes = ss_session->hw_end_ptr - ss_session->hw_rd_ptr;
            memcpy(process_input_buff, ss_session->hw_rd_ptr, copy_bytes);
            memcpy(process_input_buff + copy_bytes, ss_session->hw_start_ptr,
                (ss_session->buff_sz - copy_bytes));
            updated_rd_ptr = ss_session->hw_start_ptr + (ss_session->buff_sz - copy_bytes);
            ss_session->hw_rd_ptr = updated_rd_ptr;
            ss_session->unread_bytes -= ss_session->buff_sz;
        } else {
            memcpy(process_input_buff, ss_session->hw_rd_ptr, ss_session->buff_sz);
            ss_session->hw_rd_ptr += ss_session->buff_sz;
            ss_session->unread_bytes -= ss_session->buff_sz;
        }

        ST_DBG_FILE_WRITE(ss_fd_kw_det, process_input_buff, ss_session->buff_sz);
        pthread_rwlock_unlock(ss_session->pcm_rw_lock);
...
exit:
     ...
     */
    pthread_mutex_lock(&ss_session->st_ses->ss_det_lock);
    ...
    pthread_cond_signal(&ss_session->st_ses->ss_det_cond);
    pthread_mutex_unlock(&ss_session->st_ses->ss_det_lock);

    /* Reinit the module */
    ALOGV("%s: Issuing capi_set_param for param %d", __func__, SVA_ID_REINIT_ALL);
    ...
    voiceprint2_result_t *result_cfg_ptr, unsigned int *det_status)
{
    ...
    ALOGV("%s: Issuing capi_get_param for param %d", __func__, VOICEPRINT2_ID_RESULT);
    rc = ss_session->capi_handle->vtbl_ptr->get_param(ss_session->capi_handle,
    ...
    if (result_cfg_ptr->is_detected) {
        *det_status = USER_VERIFICATION_SUCCESS;
        ss_session->confidence_score = (int32_t)result_cfg_ptr->combined_user_score;
    }
...
static int start_user_verification(st_arm_second_stage_t *st_sec_stage)
{
    ...
    uv_cfg_ptr->sva_uv_confidence_score = ss_session->st_ses->hw_ses_current->user_level;
    ...
    ALOGV("%s: Issuing capi_set_param for param %d", __func__, VOICEPRINT2_ID_SVA_UV_SCORE);
    rc = ss_session->capi_handle->vtbl_ptr->set_param(ss_session->capi_handle,
    ...
    while (!ss_session->exit_buffering) {
        ...
        ST_DBG_FILE_WRITE(ss_fd_user_ver, process_input_buff, ss_session->buff_sz);
        pthread_rwlock_unlock(ss_session->pcm_rw_lock);
        ret = process_frame_user_verification(ss_session, process_input_buff, stream_input,
            result_cfg_ptr, &det_status);
        pthread_rwlock_rdlock(ss_session->pcm_rw_lock);
        if (ret) {
            ALOGE("%s: Processing through capi wrapper failed", __func__);
            break;
...
exit:
    ...
    /* Reinit the module */
    ALOGV("%s: Issuing capi_set_param for param %d", __func__, VOICEPRINT2_ID_REINIT);
    rc = ss_session->capi_handle->vtbl_ptr->set_param(ss_session->capi_handle,
...
static void *buffer_thread_loop(void *st_second_stage)
{
    ...
    while (!ss_session->exit_thread) {
        ...
        if (st_sec_stage->ss_info->sm_detection_type == ST_SM_TYPE_KEYWORD_DETECTION) {
...
int st_second_stage_start_session(st_arm_second_stage_t *st_sec_stage)
{
    ...
    if (st_sec_stage->ss_info->sm_detection_type == ST_SM_TYPE_KEYWORD_DETECTION) {
        ...
        ALOGV("%s: Issuing capi_set_param for param %d", __func__, SVA_ID_THRESHOLD_CONFIG);
        rc = ss_session->capi_handle->vtbl_ptr->set_param(ss_session->capi_handle,
        ...
        if (CAPI_V2_EOK != rc) {
            ret = -EINVAL;
            ALOGE("%s: set_param SVA_ID_REINIT_ALL failed, result = %d", __func__, rc);
            goto exit;
        }
    } else if (st_sec_stage->ss_info->sm_detection_type == ST_SM_TYPE_USER_VERIFICATION) {
        ...
        threshold_cfg->user_verification_threshold = ss_session->confidence_threshold;
        ...
        if (CAPI_V2_EOK != rc) {
            ret = -EINVAL;
            ALOGE("%s: set_param VOICEPRINT2_ID_THRESHOLD_CONFIG failed, result = %d",
                __func__, rc);
            goto exit;
        }
    }
...
int st_second_stage_module_init(st_arm_second_stage_t *st_sec_stage, void *lib_name)
{
    int ret = 0;
    ...
    if (st_sec_stage->ss_info->sm_detection_type == ST_SM_TYPE_KEYWORD_DETECTION)
        ss_session->capi_handle = calloc(1, sizeof(capi_v2_t) + sizeof(char *));
    else if (st_sec_stage->ss_info->sm_detection_type == ST_SM_TYPE_USER_VERIFICATION)
        ss_session->capi_handle = calloc(1, sizeof(capi_v2_t) + (3 * sizeof(char *)));
