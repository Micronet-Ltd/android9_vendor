...
#define PA_QST_DBUS_READ_TIMEOUT_MS 1000

/*
 * DBUS MODULE INTERFACE VERSION 101 adds support for
 * 1. Module Iterface Version API:
 * Method: GetInterfaceVersion()
 * 2. Non-Blocking Read APIs:
 * Signal: ReadBufferAvailableEvent
 * Method: RequestReadBuffer()
 */
...
int pa_qst_read_buffer(const pa_qst_handle_t *mod_handle,
    ...
    if (m_data->interface_version < PA_QST_DBUS_MODULE_IFACE_VERSION_101) {
        ...
        end_time = start_time + (PA_QST_DBUS_READ_TIMEOUT_MS * G_TIME_SPAN_MILLISECOND);
        g_mutex_lock(&ses_data->mutex);
