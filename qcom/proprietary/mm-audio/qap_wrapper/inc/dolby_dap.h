...
#define QAP_DAP_CPDP_PROCESS_OPTIMIZER_NB_BANDS_MAX 20
#define QAP_DAP_CPDP_AUDIO_OPTIMIZER_NB_BANDS_MAX 20u
...
typedef struct dap_channel_format {
    /* number of ground channels */
    int ground_channel_num;
    /* number of height channels */
    int height_channel_num;
    /* is lfe channel present */
    int lfe;
    /* ground + height + lfe */
    int channel_num;
...
typedef enum {
    ...
    /* set process optimizer bands */
    QAP_DAP_PARAM_PROCESS_OPTIMIZER_BANDS_SET,

    /* set audio optimizer bands */
    QAP_DAP_PARAM_AUDIO_OPTIMIZER_BANDS_SET,
...
typedef struct process_opt {
    unsigned nb_bands;
    unsigned center_frequencies[QAP_DAP_CPDP_PROCESS_OPTIMIZER_NB_BANDS_MAX];
    int gains[QAP_DAP_MAX_DAP_IO_CHANNELS][QAP_DAP_CPDP_PROCESS_OPTIMIZER_NB_BANDS_MAX];
} process_optimizer_t;

typedef struct audio_opt {
    unsigned nb_bands;
    unsigned center_frequencies[QAP_DAP_CPDP_AUDIO_OPTIMIZER_NB_BANDS_MAX];
    int gains[QAP_DAP_MAX_DAP_IO_CHANNELS][QAP_DAP_CPDP_AUDIO_OPTIMIZER_NB_BANDS_MAX];
} audio_optimizer_t;
...
typedef struct {
    ...
    /* set process optimizer bands*/
    process_optimizer_t process_optimizer;

    /* set process optimizer bands*/
    audio_optimizer_t audio_optimizer;
