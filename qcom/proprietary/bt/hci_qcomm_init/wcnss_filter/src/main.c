...
void set_debug_properties() {
    ...
    ALOGV("%s: property_get: vendor.wc_transport.force_special_byte: %d", __func__, ret);
    ...
    enabled = false;
#endif
    ret = property_get("vendor.wc_transport.out_ringbuf_log", value, NULL);
    ALOGV("%s: property_get: vendor.wc_transport.ringbuf: %d", __func__, ret);

    if (ret) {
        enabled = (strcmp(value, "false") ==0) ? false : true;
        ALOGV("%s: vendor.wc_transport.out_ringbuf_log: %s, enabled: %d ",
            __func__, value, enabled);
    }
    ...
    ret = property_get("persist.vendor.service.bdroid.fwsnoop", value, "false");
    ALOGV("%s: property_get: persist.vendor.service.bdroid.fwsnoop: %d", __func__, ret);

    if (ret) {
        enabled = (strcmp(value, "false") == 0) ? FALSE : TRUE;
        ALOGV("%s: persist.vendor.service.bdroid.fwsnoop: %s, enabled: %d ",
            __func__, value, enabled);
        g_fwsnoop = enabled;
    }
}
