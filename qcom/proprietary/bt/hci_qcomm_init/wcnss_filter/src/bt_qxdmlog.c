...
bool is_force_hcidump_enabled() {
    ...
    ret = property_get("vendor.wc_transport.force_hcidump", value, NULL);
    ALOGI("%s: property_get: vendor.wc_transport.force_hcidump: %d", __func__, ret);

    if (ret) {
        enabled = (strcmp(value, "false") ==0) ? FALSE : TRUE;
        ALOGV("%s: vendor.wc_transport.force_hcidump: %s, enabled: %d ",
...
static int is_unified_logging_enabled() {
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.unified_bt_logging", value, "false");
...
bool is_crashdump_enabled(void)
{
    ...
    /* Check if crash dump is enabled */
    ret = property_get("vendor.wc_transport.crashdump", value, NULL);
    ALOGV("%s: property_get: vendor.wc_transport.crashdump: %d", __func__, ret);
    ...
    if (ret) {
        ...
        ALOGV("%s: vendor.wc_transport.crashdump: %s, crashdump_enable: %d ",
