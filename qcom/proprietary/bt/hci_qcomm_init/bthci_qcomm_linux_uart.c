...
#ifdef ANDROID
    ret = property_get("ro.vendor.qualcomm.bt.hci_transport", transport_type, NULL);
    if(ret == 0)
        printf("ro.vendor.qualcomm.bt.hci_transport not set\n");
    else
        printf("ro.vendor.qualcomm.bt.hci_transport: %s \n", transport_type);
