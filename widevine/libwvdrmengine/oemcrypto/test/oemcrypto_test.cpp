...
TEST_F(OEMCryptoKeyboxTest, GenerateDerivedKeysFromKeyboxLargeBuffer) {
  ...
  ASSERT_EQ(OEMCrypto_SUCCESS,
            OEMCrypto_GenerateDerivedKeys(s.session_id(), &mac_context[0],
...
TEST_F(OEMCryptoSessionTests, DecryptUnencryptedNoKey) {
  ...
  // Decrypt the data
  sts =
      OEMCrypto_DecryptCENC(s.session_id(), &in_buffer[0], in_buffer.size(),
                            false, &encryptionIv[0], 0, &destBuffer, &pattern,
                            OEMCrypto_FirstSubsample | OEMCrypto_LastSubsample);
  ASSERT_EQ(OEMCrypto_SUCCESS, sts);
...
TEST_F(OEMCryptoLoadsCertificate, RSAPerformance) {
  ...
  ASSERT_NO_FATAL_FAILURE(s.open());
  ASSERT_EQ(OEMCrypto_SUCCESS,
            OEMCrypto_LoadDeviceRSAKey(s.session_id(), &wrapped_rsa_key_[0],
                                       wrapped_rsa_key_.size()));
...
TEST_F(OEMCryptoUsesCertificate, GenerateDerivedKeysLargeBuffer) {
  ...
  ASSERT_EQ(OEMCrypto_SUCCESS,
            OEMCrypto_DeriveKeysFromSessionKey(
                session_.session_id(), &enc_session_key[0],
                enc_session_key.size(), &mac_context[0], mac_context.size(),
...
TEST_P(UsageTableTestWithMAC, ReloadOfflineLicenseWithTerminate) {
  ...
  Restart();
  ASSERT_EQ(OEMCrypto_SUCCESS,
            OEMCrypto_LoadUsageTableHeader(&encrypted_usage_header_[0],
...
TEST_F(UsageTableTest, UpdateFailsWithNullPtr) {
  ...
  ASSERT_NE(OEMCrypto_SUCCESS,
            OEMCrypto_UpdateUsageEntry(
                ...
                &buffer[0], &entry_buffer_length));
  ASSERT_NE(OEMCrypto_SUCCESS,
            OEMCrypto_UpdateUsageEntry(
                s.session_id(), &encrypted_usage_header_[0],
...
TEST_F(UsageTableTest, ReloadUsageTableWithSkew) {
  ...
  ASSERT_EQ(OEMCrypto_WARNING_GENERATION_SKEW,
            OEMCrypto_LoadUsageTableHeader(&old_usage_header_1_[0],
